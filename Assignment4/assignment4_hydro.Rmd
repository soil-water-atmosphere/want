...
```{r}
# The data: This will construct a dataframe containing a small example of a spatially measured quantity called T. This is the value to be interpolated and the location of this value is described by a x and y position.
tointerp = data.frame(
...
<div class="question">
12.Construct the new interpolation function, idf2 and idf3, using 0.5 and 4 as value for the argument power. Discuss the differences and similarities with all the previously used interpolation functions.
</div>
...
<div class="question">
13.Choose the smoothest interpolator that still respects the data and plot two dimensional model domain of TwoDModel with the interpolated transmissivity values. 
</div>
...
In the TwoDmodel the constant value of $kD=60\frac{m^2}{d}$ was used as value for the transmissivity. We will now construct a model that is identical to the TwoDmodel, except for the interpolation function of the transmissivities in the internal flux of the new model.

* Call the new model InterTwoDModel
...
<div class="answer">
If the interpolation function is implemented correctly, a change of the countour line positions in the model domain should be noticed.
```{r}
...
<div class="question">
15.Copy the TwoDModel, call it TwoDModelWell and implement a well at location x=1000 and y = 1000 with an extraction rate of 1000 $m^2$. Finally, solve the model and create a 2D and 3D plot. You can use the 'summary(modelname)' function to check what is implemented in the model.
</div>
...
set.discretisation(TwoDModelWell, nodes=list(type="geomgen", optdist=welldist, verbose=TRUE), method="FE")
...
<div class="question">
18.Copy the TwoDModel, call it TwoDModelRiver and implement a canal (straight line) between the points (400, 500) and (1500, 1000) and assign it an extraction rate of 1 $m$. Finally, solve the model and create a 2D and 3D plot.
</div>
...
<div class="answer">
The solution is more precise and realistic in the neighbour hood of the river, due to the change in discretisation.
```{r}
# Copy and assign new name to the model.
TwoDModelRiver = copy.model(model=TwoDModel)
...
In the last part of this assignment (simplified) flow through a dike, as shown in figure 2, will be modelled.
...
In figure 3 a schematised vertical cross section of a dike is presented. Note the different role of the y-direction compared to the x-direction. From figure 3 can be seen that the y-to-x-ratio is not equal to one. This can be graphically observed in this particular case, but this is not always so clear. Be aware that this y-to-x-ratio exists in this model. 
...
This model will use head (i.e. pressure + elevation) as state variable. The internal flux will be added to the model as defined in the R-code block below.
```{r}
...
<div class="answer">
The hydraulic conductivity is positively related to pressure in the unsaturated zone. Pressure is coupled with head via elevation. As pressure gets more negative with increasing distance from the saturated zone, the hydraulic conductivity decreases in magnitude too. Therefore, the flow is overestimated using the current internal flux function.
</div>
...
* The horizontal top of the dike receives a rechage flux of 0.001 $mm$

<div class="question">
22.Create a new model object and add the boundary conditions to your model. Give for each of these boundary conditions an interpretation. What boundary condition is used on the remaining boundary segments?
</div>
...
* Top:          Rainfall infiltrating in the dike
...
<div class="question">
23.Add a proper discretisation to the model. Use for this the geomden method, with an opdist of 0.5 and the proper y-to-x-ratio. Use the Finite Elements discretisation method. Add a randomly filled plot of the discretised domain.
</div>

<div class="answer">
```{r}
# Set the discretisation.
set.discretisation(DikeFlow, nodes=list(type="geomgen", optdist=0.5, ytoxratio=5, verbose=TRUE), method="FE")
...
<div class="question">
24.Solve and run the model. Make a plot with 10 contour lines between 0 and 1 and add fluxvectors. Where do you observe the largest fluxes?
</div>

<div class="answer">
The largest flux is observed at the lower west boundary.
```{r}
# Solve
...
Apart from flow velocities, the distribution of pressure in the dike is also important in order to investigate the stability of the dike. The standard use of the plot function as implemented in the FVFE2D package is with fill= "state" and will show in this case head distribution and not pressure distribution. However, one can assign a function as fill argument for the plotting function. 

<div class="question">
25.Define the pressure function and create a contour plot of the pressure distribution in the dike. Note that the zero contour line represents the border between the saturated and unsaturated zone. 
</div>
...
This last question lets you play with the dike model. Convince yourself that with the help of the model that you have built you can investigate the flow through a dike for the following examples:
