---
title: "FVFE practical 3 - Finite volumes, 1D steady and transient"
date: "24/08/2018"
output:
  html_document:
    css: want.css
...
## Learning goals
* Understand unsaturated flow state variables and formulas *(Introduction)*
...
* Set up a stationary and transient unsaturated flow model *(Exercise 3,5)*
* Apply initialisation in a model run *(Exercise 3)*
...
Unsaturated water flow occurs in the zone between the soil surface and the (saturated) groundwater table. The flux in this part of the hydrological cycle is predominantly vertical. For that reason a 1D-model will be constructed along the vertical axis, denoted by z. We will follow the (historical) convention to choose the z-direction upwards. We will try to follow the convention that if we want to make a plot of e.g. the distribution of the soil moisture $\theta(z)$ in the soil column, $\theta$ is plotted on the horizontal axis and z on the vertical axis. Such a plot is called a *moisture profile*. A similar plot can be made for the water pressure head $\psi(z)$. The vertical distribution of $\psi(z)$ is called a *pressure profile*. In unsaturated zone modelling we generally work with units of cm and days.

For the unsaturated zone several state variables are possible:

*   $\theta(z)$ : moisture content (-)  
...
In these assignments we will choose $\psi(z)$ as state variable.

Using the Darcy formula, the internal flux in the unsaturated zone can now be expressed in terms of the pressure and the hydraulic conductivity.

<div class="exercise"> 
Write down the formula for unsaturated Darcy flux.
</div>
...
<div class="exercise">
Cleaning the workspace and loading the FVFE1D package:
```{r}
rm(list = ls())
library(FVFE1D)
...
Before constructing models for flow in the unsaturated zone an overview of the units would be practical.
...
# Exercise 1: Exploring the hydraulic relations  

Not all the quantities cited above are independent.
For each soil there are unique relations between $\theta$ , $\psi$ and $k$.  
You can explore these relations by running the R-script called **__soilprofiles.R__** separately. You have to load the file *Staring.Rdata* into the same folder. 
...
<div class="exercise">
Using the plots created by `soilprofiles.R`, answer the following questions (by clicking the wheel in the upper left you can change the soil type).
...
*   Which soil do you expect to enable capillary rise from the groundwater best? You will look further into this in exercise 5.
</div> 
...
## Exercise 2: Exploring the equilibrium profiles 
Even the situation where there is no flow through the soil is interesting. The corresponding profiles of $\psi$ and $\theta$ are called equilibrium profiles. 

The code chunk below is mostly preprogrammed. It illustrates the pressure and moisture content profile at equilibrium with the groundwater for a certain soil.   
The surface level is set to 0.0 cm. Assume that the groundwater table is at 1 m (100 cm) depth. The internal flow is defined to be positive upwards.

<div class="exercise">
2.1 What are the boundary conditions at the lower(left) and upper (right) end of the domain? Fill out AA and BB in the chunk.  
2.2 Derive for the internal flux the differential equation for $\psi$ for the equilibrium situation. Use this to fill out XX and YY in the chunk.  
</div>
...
<div class="exercise">
2.3 Select one of the soils to plot the pressure and moisture content profile for the equilibrium situation.  
...
plot(theta.seq,z.seq,type="l",xlab="theta",ylab="z",col="red",lwd=2,
     ...
     main=)
# title(main=pacte('tst', outer = TRUE)
...
<div class="answer">
2.1 The domain runs from -100 cm (the groundwater table) to 0 cm (the ground surface).   
2.2 Because we have a no flow situation (soil moisture in equilibrium with the groundwater table), we can write $q(z)=-k(\psi) \left ( \frac{d\psi}{dz}+1 \right )=0$     
...
<div class="exercise"> 2.4 The pressure profile is the same for all soils. Can you explain why? </div> 
...
<div class="answer">
As determined earlier, when $q(z)=0$, $\frac{d\psi}{dz}=-1$ which is just a constant. It simply means that when $z$ is increased by 1, $\psi$ is increased by -1. This is independent of the soil type.   
</div>

<div class="exercise">
2.5 What is the difference between an equilibrium situation and a stationary flow situation, for the unsaturated zone case?
</div>
...
## Exercise 3: Stationary unsaturated zone model  
We start by building a stationary model for flow in the unsaturated zone. The following code chunk is partly preprogrammed but still needs to be finished.  
...
do.initialize(UnsatModel,Psi.equi)
...
set.isacceptable(UnsatModel,Psi.in.range) #the set.isacceptable function limits the possible values that the state can become

######## solving the model ####################
solve.steps(UnsatModel,verboselevel=1)

######## plotting #############################
#we will create two plots, of the psi profile and the theta profile
Psi.seq = dataframe.states(UnsatModel)$state
z.seq = dataframe.states(UnsatModel)$x
...
################### make the model #################
UnsatModel = newFLOW1D(domain,unsatdarcy.flux,"Unsaturated zone, 1D stationary")
...
# right = flux at the surface. Choose XXXX such 
# that it corresponds to 1 mm/day of recharge 
set.BC.fixedflux(UnsatModel,"right",0.1)  ##0.1cm/d=1mm/d. Although this flow is flow is downwards, we define it positive because it goes into the domain.
...
Here we are clearly not in a no-flow situation any more: the $\psi$ profile diverts from the equilibrium profile. The topsoil is wetter than equilibirum, so there is downward flow.
...
## Exercise 4: Application of the model
...
<div class="exercise">
4.1 Ponding occurs when the maximum downward flow capacity of the soil is exceeded, so that the soil is saturated to the surface.    
Fill out the following table. You can use the above code chunk. 
...
<div class="exercise">
4.2 Fill in the maximum capillary rise per soil type.
...
<div class="exercise"> 
4.3 Calculate the total amount of water stored in the unsaturated zone, with a groundwater table at 1m below surface, for a situation with precipitation and for a situation with evapotranspiration excess.

<button type="button" onclick="showHint(4)">Show Hint</button>
<p id="Q4"> </p>

Total stored volume| Coarse sand | Loam | Heavy clay | Sandy peat | 
----|----|----|----|----|  
...
```{r}
wat.at.nodes = Soil$theta.fun(UnsatModel$states)
...
* The top forcing (precipitation surplus) is initially set at 0 (line 477). Later on (from line 491), a time dependent top forcing is created. This function results in precipitation for the time until T1, followed by evaporation between T1 and T2, and zero top forcing from T2 to the end.  

Remove "eval=FALSE", then you can run the chunk. 
</div>
...
##
UnsatTrans = newFLOW1D(domain,unsatdarcy.flux)

##
add.spatialflux(UnsatTrans,changeinstorage,"change in storage")

##
set.BC.fixedstate(UnsatTrans,"left",0)
...
<div class="answer">
The storage change is the change in moisture content; $\frac{\partial \theta}{\partial t}$. The storage function can now written as: $-\frac{\theta(\psi^{new})-\theta(\psi^{old})}{\Delta t}$. The moisture content can be derived using the __Soil$theta.fun()__ function which is a function of the calculated state $\psi$ (see chunk). 
The code chunk below sets up the transient unsaturated zone model.
...
<div class= "exercise">    
5.2    Change the soil type. How does the soil moisture behaviour change?      
5.3    Change the method into Finite Elements. What is the most significant change you observe? 
</div>

<div class="answer">
5.2 This chunk runs the model with soil B11, heavy clay.
```{r animation2, echo=FALSE, fig.show='animate', aniopts='controls'}
###Running with another soil type: B11 heavy clay
UnsatTransClay=copy.model(UnsatTrans)
...
## ------------------------------------------------------------------------
do.initialize(UnsatTransClay,Psi.equi)
oldpsifun = Psi.equi
...
while(time < end.time)
{
  ...
  control=solve.steps(UnsatTransClay)
  
  curresult = dataframe.states(UnsatTransClay)
  ##saving intermediate results
  psi.atsurface = c(psi.atsurface,curresult$state[length(nodes)])
  savestorage = c(savestorage,dataframe.balance(UnsatTransClay)[2,4])
  savepercolation = c(savepercolation,dataframe.boundaries(UnsatTransClay)[1,4])
  ##saving intermediate results
  Psi.seq = dataframe.states(UnsatTransClay)$state
  z.seq = dataframe.states(UnsatTransClay)$x
  Psi.equi.seq = Psi.equi(z.seq)
  Psi.range=range(Psi.seq,Psi.equi.seq)
  theta.seq = Soil$theta.fun(Psi.seq)
  old.par= par(no.readonly=TRUE)
...
5.2 With the same top forcing, the clay soil (B11) takes more time to reach equilibrium than the coarse sand soil (B5). This happens because downward drainage of the excess water is slower in clay. You can see this if you plot the `savepercolation.B5` and `savepercolation.B11` vectors: clay has a much lower percolation.     
![](Ex6Answer.png)


5.3 If you run the model using finite elements (not shown in the chunks), the dynamics of the soil moisture profile change very little. However, if you look at the fluxes or storage changes that happen in each in timestep (using one of the dataframes that are saved during the while loop), small differences are visible.
</div>

## Exercise 6: Changed top forcing function

Now you will implement a more 'realistic' top forcing. The R-file `topforcing.R` creates an artificial rainfall-evaporation pattern with the functions 'rainfun' and 'evapfun'.  Reset the soiltype to B5.   

<div class="exercise">
6.1   Create a separate chunk for making a plot of the precipitation, evapotranspiration and net input (rainfall surplus) provided by `topforcing.R`.
</div>
...
plot(timesteps,rainseq,type="h",col='blue',ylim=c(-1,3),ylab='data (cm/d)',xlab='time (days)')
...
<div class="exercise">
6.2 Run the transient model with this top forcing. You can comment the lines of the first forcing function and uncomment those of the second one; either make a new chunk or reuse the previous one.      
...
##THE NEW TRANSIENT MODEL
## ------------------------------------------------------------------------
UnsatNewTop = newFLOW1D(domain,unsatdarcy.flux)

##ADDING THE STORAGE FLUX
## ------------------------------------------------------------------------
add.spatialflux(UnsatNewTop,changeinstorage,"change in storage")

##SETTING THE BOUNDARY CONDITION AT THE WATER TABLE; FIXED STATE = 0 cm
## ------------------------------------------------------------------------
set.BC.fixedstate(UnsatNewTop,"left",0)
...
## ------------------------------------------------------------------------
topforcing = 0
set.BC.fixedflux(UnsatNewTop,"right","topforcing")

##DISCRETISE THE MODEL WITH NODES, NODAL SPACING OF 1 CM, USING FINITE VOLUMES(FOR NOW)
## ------------------------------------------------------------------------
nodes = seq(from=domain[1],to=domain[2],by=1)
set.discretisation(UnsatNewTop,nodes,"FV")
...
set.isacceptable(UnsatNewTop,Psi.in.range)
...
##STARTING THE SIMULATIONS
## ------------------------------------------------------------------------
do.initialize(UnsatNewTop,Psi.equi)
oldpsifun = Psi.equi
...
while(time < end.time)
{
  ...
  control=solve.steps(UnsatNewTop)
  
  curresult = dataframe.states(UnsatNewTop)
  ##saving intermediate results
  psi.atsurface = c(psi.atsurface,curresult$state[length(nodes)])
  savestorage = c(savestorage,dataframe.balance(UnsatNewTop)[2,4])
  savesavepercolation = c(savepercolation,dataframe.boundaries(UnsatNewTop)[1,4])
  ##saving intermediate results
  Psi.seq = dataframe.states(UnsatNewTop)$state
  z.seq = dataframe.states(UnsatNewTop)$x
  Psi.equi.seq = Psi.equi(z.seq)
  Psi.range=range(Psi.seq,Psi.equi.seq)
  theta.seq = Soil$theta.fun(Psi.seq)
  old.par= par(no.readonly=TRUE)
...
## Exercise 7: Actual evapotranspiration
Evapotranspiration is an important environmental condition for processes near the soil surface, such as the development of vegetation. In the previous exercise, we used evaporation/evapotranspiration as a given flux. In reality, the evapotranspiration depends on the actual soil moisture levels at the surface.    
The vegetation can only transpire optimally when there is sufficient moisture in the soil which is 'easily' available for the vegetation. The drier a soil becomes, the harder the vegetation can extract water from it, since the roots have to overcome a large negative pressure.  This process can be simulated with the so called Feddes (retired Prof. from WUR) function.  

The code chunk below illustrates such a function.The shape of the Feddes function varies for different vegetation types.

```{r}
Feddes.habs = c(0.1,100,1000,16000)
Feddes.red = c(0,1,1,0)
Feddes.fun= approxfun(Feddes.habs,Feddes.red,rule=2)
plot(Feddes.habs,Feddes.fun(Feddes.habs),type="l",lwd=3,xlim=range(Feddes.habs),log="x",
     ylab="Root uptake reduction",xlab="Suction (-h)")
abline(v=Feddes.habs,col="red")
...
The Feddes function thus relates the root water uptake at a certain depth with the pressure (suction) at that depth. This makes transpiration dependent on the rooting distribution of the vegetation.

Suppose that a vegetation has an evenly distributed rooting system over the first 25 cm.  
For this the actual evapotranspiration could be calculated with:  
$\text{extraction}(z,\psi)_{roots}= - \text{roots.fun}(z)*\text{Feddes.fun}(z,\psi)*\text{max.extraction rate}_{roots}$  

Here, the _roots.fun_ describes the proportion of the roots growing at a certain depth. The _max. root extraction rate_ is equal to the **potential evapotranspiration** and is extracted along the total rooting depth for this vegetation. 

<div class="exercise">
7.1 Complete the following chunk of code implementing the actual evapotranspiration.     
*   Use a potential evapotranspiration of 0.3 cm/d.    
*   Calculate the max.root.rate.       
*   Finish the roots extraction function (**roots**) to simulate the actual evapotranspiration.      
*   First simulate a situation with with no precipitation starting with an equilibrium situation.   

7.2 Evaluate the effect of the actual evapotranspiration. What happens if you change the Feddes function? For example, change the `Feddes.h` vector to `c(0,-10,-40,-16000)` (which would be a very wet-loving plant) or adapt the rooting depth.
</div>

```{r eval=FALSE}
UnsatFeddes=copy.model(UnsatTrans)
domain = c(-50,0)

Feddes.h = c(0,-100,-1000,-16000)
Feddes.red = c(0,1,1,0)
Feddes.fun= approxfun(Feddes.h,Feddes.red,rule=2)
max.roots.rate = XXXX
root.depth = approxfun(c(0,-25,-25.01),c(1,1,0),rule=2)

roots = function(x,state)
{
  return(-root.depth(x)*Feddes.fun(state)*XXXX)
}

add.spatialflux(UnsatFeddes,rate = roots,name = 'roots')

Psi.equi = approxfun(c(domain[1],domain[2]),c(domain[2],domain[1]))
do.initialize(UnsatFeddes,Psi.equi)
Psi.in.range = function(z,Psi) # it has 2 arguments; the position and the state
{
  if(Psi<=-1E5) return(FALSE)
  else return(TRUE)
}
set.isacceptable(UnsatFeddes,Psi.in.range)

oldpsifun = Psi.equi
begin.time = 0
end.time = 10 #you may adjust this
delt = 1/2
time = begin.time+delt
...
{
  topforcing = 0.0
  control=solve.steps(UnsatFeddes)
  
  curresult = dataframe.states(UnsatFeddes)
  Psi.seq = dataframe.states(UnsatFeddes)$state
  z.seq = dataframe.states(UnsatFeddes)$x
  Psi.equi.seq = Psi.equi(z.seq)
  Psi.range=range(Psi.seq,Psi.equi.seq)
  theta.seq = Soil$theta.fun(Psi.seq)
  #old.par= par(no.readonly=TRUE)
...
<div class="answer">
7.1 The code chunk below implements the actual evapotranspiration function.  

```{r echo=FALSE}
UnsatFeddes=copy.model(UnsatTrans)
domain = c(-50,0)

##functions to calculate actual evapotranspiration with constant pot.evapotranspiration of 0.3 cm/d
Feddes.h = c(0,-100,-1000,-16000)
Feddes.red = c(0,1,1,0)
Feddes.fun= approxfun(Feddes.h,Feddes.red,rule=2)
max.roots = 0.3/40 #the actual root uptake is calculated separately for each depth, so each depth get its 'proportion' of the potential evapotranspiration.
root.depth = approxfun(c(0,-40,-40.01),c(1,1,0),rule=2)
roots = function(x,state)
{
  return(-root.depth(x)*Feddes.fun(state)*max.roots)
}
...
do.initialize(UnsatFeddes,Psi.high)
Psi.in.range = function(z,Psi) # it has 2 arguments; the position and the state
{
  if(Psi<=-1E5) return(FALSE)
  else return(TRUE)
...
while(time < end.time)
{
 ##topforcing = forcing.fun(time)
  topforcing = 0.0
  control=solve.steps(UnsatFeddes)
  wbal = dataframe.balance(UnsatFeddes)
  evap.act = c(evap.act,wbal[3,4]) #net root extraction
  storage.act = c(storage.act,wbal[2,4])
  caprise.act = c(caprise.act,dataframe.boundaries(UnsatFeddes)[1,4])
  curresult = dataframe.states(UnsatFeddes)
  Psi.seq = dataframe.states(UnsatFeddes)$state
  z.seq = dataframe.states(UnsatFeddes)$x
  Psi.equi.seq = Psi.equi(z.seq)
  Psi.range=range(Psi.seq,Psi.equi.seq)
  theta.seq = Soil$theta.fun(Psi.seq)
  roots.seq=dataframe.externalfluxes(UnsatFeddes)$roots
  time = time+delt
  oldpsifun = approxfun(curresult$x,curresult$state,rule=2)
}
...
<div class="answer">
7.2 As can be seen in the fluxes graph, the evapotranspiration for the original Feddes function is about 0.12 cm/d, so it is limited compared to the potential flux of 0.3 cm/d. This happens because the soil is wetter than optimal (h>-100). The evapotranspiration is first drawn from the storage (positive storage flux, so water extracted from storage).As the soil becomes drier, the evapotranspiration increases as it comes closer to its optimal range. Finally, evapotranspiration is balanced by capillary rise. 

When another Feddes function is implemented for a more wet-loving vegetation, the evapotranspiration flux is higher. However, the flux decreases over time, as the soil becomes too dry.
</div>

## Notes for WANT team
Exercise 4 is very nice for the applied track. Maybe for the in-depth track we can replace this by a more modelling-related question?
In 5.3 it is asked to run with FE and look at the differences. This is interesting to go some deeper into (for in depth track?): does FV or FE really matter for your results, except for smoothness? In which cases does it matter?
The term recharge and infiltration were used a bit confusingly, I made sur infiltration was only inflow at the surface, and used recharge and percolation for flows to the groundwater. 
Do we wannt to add some meteo in the last question(s)? How?