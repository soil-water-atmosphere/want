...
subtitle: "Finite volumes, 1D steady and transient"
date: "24/08/2018"
output:
  html_document:
    css: want.css
...
6. Because we have a no-flow situation (soil moisture in equilibrium with the groundwater table), we can write $q(z)=-k(\psi) \left ( \frac{d\psi}{dz}+1 \right )=0$     
...
<div class="exercise">
11. Ponding occurs when the maximum downward flow capacity of the soil is exceeded, so that the soil is saturated to the surface.    
...
<div class="exercise">
12. Fill in the maximum capillary rise per soil type.
...
<div class="exercise"> 
13. Calculate the total amount of water stored in the unsaturated zone, with a groundwater table at 1m below surface, for a situation with precipitation and for a situation with evapotranspiration excess.
...
16.    Change the method into Finite Elements. What is the most significant change you observe? 
</div>
...
16. If you run the model using finite elements (not shown in the chunks), the dynamics of the soil moisture profile change very little. However, if you look at the fluxes or storage changes that happen in each in timestep (using one of the dataframes that are saved during the while loop), small differences are visible.
</div>
...
<div class="exercise">
17.   Create a separate chunk for making a plot of the precipitation, evapotranspiration and net input (rainfall surplus) provided by `topforcing.R`.
</div>
...
<div class="exercise">
18. Run the transient model with this top forcing. You can comment the lines of the first forcing function and uncomment those of the second one; either make a new chunk or reuse the previous one.      
...
20. Evaluate the effect of the actual evapotranspiration. What happens if you change the Feddes function? For example, change the `Feddes.h` vector to `c(0,-10,-40,-16000)` (which would be a very wet-loving plant) or adapt the rooting depth.
</div>
...
<div class="answer">
19. The code chunk below implements the actual evapotranspiration function.  
...
<div class="comment">
Comments:
* Section 4 is very nice for the applied track. Maybe for the in-depth track we can replace this by a more modelling-related question?
* In section 5, question 16 it is asked to run with FE and look at the differences. This is interesting to go some deeper into (for in depth track?): does FV or FE really matter for your results, except for smoothness? In which cases does it matter?
* Do we want to add some meteo in the last question(s)? How?
</div>