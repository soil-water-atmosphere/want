...
*   Which soil is expected to be able to maintain the largest upward flux from the groundwater towards the surface due to capillary rise? You will look further into this in exercise 5.
</div> 
...
Even the situation at times there is no flow through the soil is interesting. The corresponding profiles of $\psi$ and $\theta$ are called equilibrium profiles. 
...
Here we are clearly not in a no-flow situation any more: the $\psi$ profile diverts from the equilibrium profile. The topsoil is wetter than equilibrium, so there is downward flow.
...
<div class="exercise">
11. Ponding can occur when the maximum water storage capacity of the soil is exceeded, so that the soil is saturated to the surface.    
...
<div class="exercise"> 
13. Calculate the total amount of water stored in the unsaturated zone, with a groundwater table at 1m below surface, for a situation with precipitation, 1 $mm/d$, and for a situation with evapotranspiration, 1 $mm/d$, excess.
</div>
...
Remove "eval=FALSE", then you can run the chunk. The clips that will be created will probably not display in the default markdown window. Go to the dirictory where the knitted html-file is saved and open it in Google Chrome.
</div>
...
<div class="exercise">
17.   Create a separate chunk and create making a plot of the precipitation, evapotranspiration and net input (rainfall surplus) provided by `topforcing.R`.
</div>

<button type="button" onclick="showHint(5)">Show Hint</button>
<p id="Q5"> </p>
...
*   Calculate the correct value for the max.roots variable.       
