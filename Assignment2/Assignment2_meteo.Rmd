...
<span class="answer">
The storage flux should have the same dimentions as the internal flux. Thus the dimentions of the storage flux are $\frac{m^2}{s}$. The storage flux is negatively related to the rate of change in the state variable because the flow direction should always be opposite to the gradient.
</span>
...
<span class="question">
5.	Make a new model which is an exact copy of your oneDinitmodel. Call this model oneDstationmodel. Use this model to create another copy, call this copy oneDtransientmodel.
</span>

<script src="hints.js"></script>

<button type="button" onclick="showHint(215)">Show Hint</button>
<p id="Q215"> </p>
...
<span class="question">
6.	Check the documentation and try to find the function which should be used to add the storageflux function to your transient model.
</span>
...
<span class="question">
7.	Add the storageflux function to your transient model. 
</span>
...
•	a value for dt (the code equivalent of $\Delta t$): use 300 seconds as a staring value 
...
<span class="question">
8.	Include a piece of code that tells the script that the initial time is zero (use the variable name “time” for time in your code).
</span>
...
<span class="question">
9. Include a piece of code that tells the script that the time should increase with $\Delta t$.
</span>
...
<span class="question">
10. Define the lower boundary condition as described by the cooling of the surface equation and add this transient boundary condition to your transient model.
</span>

<span class="answer">
```{r}
# Due to the simplicity of the tranient boundary condition, the function can be entered as an argument directly. Although the function is called fixed state, the left boundary condition will be resetted at every time step. This way the state of the left boundary condition is fixed, but only during one time step.
...
<span class="question">
11. Add this piece of code and verify that it indeed gives the value of the state at the previous time step, $t=0$ in this case, for the locations $x=24.5$ and $x=50.5$.
</span>
...
<span class="question">
12. Add a piece of code that defines the numerical formulation of your model and solve it to obtain the profile of the state variables at $t=\Delta t$. The nodal distance should be two meters.
</span>
...
<span class="question">
13. Plot the profiles of the state variables and the internal fluxes for time $t=\Delta t$ and compare these to the profiles at $t=0$.
</span>

<button type="button" onclick="showHint(2113)">Show Hint</button>
<p id="Q2113"> </p>

<span class="answer">
```{r}
# Plot the situation at t=0
plot(oneDstationmodel, fluxplot=TRUE)

# Plot the situation at t=dt
plot(oneDtransientmodel, fluxplot=TRUE)

# The flux at the lower boundary has changed a little due to the relatively small time step. Therefore the states have changed little, at least visually. We expect the largest change in states over time at the lower boundary.
```
</span>

<span class="question">
14.	Use dataframe.balance(oneDtransientmodel) and show that the sum of the external fluxes and the storage flux are in balance with the net internal fluxes.
</span>
...
<span class="question">
15. Add a piece of code that increments the time by one time step, $\Delta t$.
</span>
...
<span class="question">
16. Update the lower boundary condition for the new situation at $t=\Delta t$.
</span>
...
<span class="question">
17. Update the function oldstate so that is takes the states from the correct time step.
</span>
...
<span class="question">
18. Solve the transient model to obtain the profile of the state variable at the new time step, $t=2\Delta t$, and plot the states and the fluxes of the new situation.
</span>
...
while (time < end.time)
{
  ...
  solve.steps(oneDtransientmodel, verboselevel = 10)
...
<span class="question">
20. Run the code. If all went well, and the endtime was chosed properly, you should see a movie of the state transitions between the initial situation and the situation after 4 hours.
...
<span class="question">
21.	 Use these ideas to make one plot showing the external flux at the lower boundary. Also, make one plot that both presents the state value at a location of 50 m above the surface and the state value at the lower boundary as a function of time.
</span>
...
plot(Results.store$t,Results.store$lvalue,xlab='Time [s]',ylab=' pot temp. [K]', ylim=c(288, 295))

# Add a line that plots the state value at 50 meters above the surface as a function of time.
lines(Results.store$t,Results.store$midstate,col='red', type = "l")
...
<span class="question">
22.	Perform a sensitivity analysis to the applied time step by modifying it from 30 seconds to 1 hour using values of 30 seconds, 10 minutes, 30 minutes, 45 minutes and 1 hour. Check the potential temperature at 50 m and the time series of the flux over the lower boundary. What is your conclusion?
</span>
...
# The time step does not have an effect on the resulting states nor on the resulting boundary flux. The increase of the time step lowers the time resolution at which the transient model is solved. If one takes a time step that is too small, computational times can become very large.
