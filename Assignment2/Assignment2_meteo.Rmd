...
# External fluxes
* External flux type 1:
  A linear decreasing radiative divergence which has a constant value of -1.74E-4 K/s from the surface to a height of 0.5 m,   above which it linearly decreases to a value of -5.55E-5 K/s at the top of the SBL.
  
# Boundary conditions
The upper temperature will be implemented as a Dirichlet boundary condition. The lower boundary condition will be implemented with a Neumann boundary condition.
For the potential temperature at the lower boundary an initial value of 291.73 K is adopted, which is decreasing with 0.5 K/h as time evolves. 
At the upper boundary a potential temperature of 295 K is prescribed which is invariable in time.
...
Before studying transient behaviour we first create stationary models that describe the start situation before the surface starts to cool.

<span class="question">
1.	Make a model for the initial situation, i.e. the situation before the surface starts to cool. Call this model oneDinitmodel. Do not forget to clear your environment and to import the FVFE1D package.
...
<span class="question">
2.	Solve the model for the initial situation to obtain values of the state variables for all volumes (nodes) in this situation. Make plots of the values of the state variable and the internal fluxes versus height.
</span>
...
<span class="question">
3.	Using dataframe.balance, check for the initial situation whether the net internal flux over the boundaries of the region defined by the volumes associated with nodes 30 and 31, equals indeed the integrated external fluxes over this region. What is your conclusion?
</span>
