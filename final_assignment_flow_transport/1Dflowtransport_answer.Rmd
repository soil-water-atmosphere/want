...
plotten = function()
{
  ...
  #kleur = hcl.colors(n =0:length(C_trans.old[,1])/length(C_trans.old[,1]),palette = "viridis", alpha = 1)
  kleur = heat.colors(n =length(C_trans.old[,1]))
  plot(nodes,C_trans.old[1,2:(length(nodes)+1)],type = "l",lwd = 2, col = grijs[1], main = "max C in time")
  ...
  grid()
  legend(x = "topright", legend = c(paste0("time:",C_trans.old[,1])),col = grijs[1:length(grijs)])
}
