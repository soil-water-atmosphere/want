...
Looking at the mass (internal) flux equation of the tranport model, see below, one can see that it contains two components, a convective part (first one) and a dispersive part (second one):
...
\end{align}
$$
Calculate both parts for two volumes in the model, one upstream of the vessel and one downstream and sum these to have the mass flux at that border of the volume. Calculate the mass balance for both volumes and look at the results (mass flux inregion and outregiom).
Some tips:
...
#save(list = c(grep(pattern = "Ctrans",x=ls(),mass.balance.old,max.C.old),value = TRUE),file = "testCtransdata.Rdata")
bewaar = function()
{
  D.sat.old <<- D.sat
  Vflow.old <<- Vflow
...
plotten = function()
{
  ...
  lines(max.C$"position.x",max.C$max.conc, col = "yellow",type ="o",pch = 20)
  grid()
}
...
Cevery.days = 100 #modulo for days to write C(x,t)
...
parnames = paste0('c("',str_flatten_comma(as.character(nodes)), collapse="", \"")

str_flatten_comma(as.character(nodes),collapse=""")

colnames(C_trans) <<- c("time",(str_flatten_comma(as.character(nodes))))
mass.balance <<- data.frame(time.step,bnd.out,storage.in,storage.out,pump.treat)

}
```

Some test cases to check if all is ok


let's go

```{r}
#load(file = "testCtransdata.Rdata")
p_t.rate = p_t.rate/2
sensitivity()
p_t.rate = 2*p_t.rate
plotten()
