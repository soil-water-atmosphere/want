...
::: question
implement the appropriate boundary conditions; the prescribed river level left and\
...
#line below was only for testing, evaluating other velocities at right border
...
::: question
1.  Create the (water) flux function(x) for the internal flux. Call it **Qflow** and make use of **approxfun** and set **rule=2**.\
...
The solute transport model will simulate the movement of the pollutione in the groundwater which comes from a continuous source.
...
Somewhere in the middle of the solute transport model a solute source (pollutant) is assigned to the model.
...
4.  `plot(your_transport_model, fluxplot = TRUE, FVstyle = T)`

5.  `mass_balance = dataframe.balance(your_transport_model)`

6.  `boundary_flux = dataframe.boundaries(your_transport_model)`
...
&= \text{Qflow}\,C - \text{Vflow}\,D\frac{\partial C}{\partial x}\\
\end{align}
$$ Calculate for both parts(convective and dispersive) for two distinctive volumes in the model, one upstream of the vessel and one downstream.
...
This means the all aspects, including the massloading are constant in time.
In reality pollutions are normally noticed much earlier.
...
Solute stored means that this is removed from the system, hence the minus sign.
...
Another important aspect is the scale of variance of the parameters.
...
The Sobol method is a global sensitivity analysis method which decomposes the variance of the output of a model into fractions which can be attributed to the input factors.
...
It is convenient to use the following structure.
Using this structure means that very easily the analysis can be carried with the aid of the **GSOBOL** package developed by former lecturer and colleague Paul Torfs.
This package depends on the following two packages: **lhs** and **truncnorm**.
These packages can be installed from CRAN and need to be installed prior to GSOBOL.
...
hist(parsample[,river_level],main = "Histogram of the sample",xlab = "river_level",col = "lightblue",prob = T)
rug(parsample[,river_level],col = "red")
lines(density(parsample[,river_level]),col = "red")
...
{
  river_level = parsample[i,1]
  k = parsample[i,2]
  p_t.rate = parsample[i,3]
  precip = parsample[i,4]
  M.load = parsample[i,5]
  Disp = parsample[i,6]
  por = parsample[i,7]
...
Looking at the results of the Sobol analysis, we can see that the parameters `river_level`, `k`, `p_t.rate`, `precip`, `M.load`, `Disp` and `por` have a different impact on the output of the model.
The sum of the first order and second order variances is equal to the total variance of the output of the model.
The first order variances indicate the impact of the parameters on the output of the model, while the second order variances indicate the interactions between the parameters.

```{r}
condexp.river_level =  condexp1(Modresult,river_level,data = output.ensemble)
...
plot(river_level,Modresult)
points(river_level,condexp.river_level$predictions,col = "red")
plot(k,Modresult)
...
```{r}
condexp.river_level =  condexp1(Modresult,river_level,data = output.ensemble.pump)
