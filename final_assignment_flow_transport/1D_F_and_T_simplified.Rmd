...
The model for stationary groundwater flow consists of one phreatic aquifer (k = 3[$\mathsf{m/d}$]). The saturated thickness is determined by the calculated head.  The total length of the model is 200 m, use a nodal distance of 5 m. At the left boundary a Cauchy type condition is assigned, simulating a river with an open water level of 3.20 m Above Mean Sea Level (AMSL) and and entrance resistance of 5 days. The right boundary is based on a water divide. On top a uniform recharge om 1 mm/d is applied.  
The (flat) bottom of the aquifer aligns with 0.0 m AMSL. This means that the calculated head is identical to the saturated thickness of the aquifer.
...
$$
Which makes this flux rate quasi 2D; since the calculated head provides the model with a (saturated) thickness. The $_D$ in $q_D$ is to indicate that it is a flux over the total saturated thickness.   
...
#boundary condition properties river
river_level = 3.2 #m AMSL
river_resistance = 5.0  #d
river_width = 4.0 #m

#boundary isohypses on the left (testing)
iso_h = 5.2 #mm
...
#adding the left boundary condition, since right is a Neumann type no action needed there
set.BC.fluxstate(model = Fmodel,where = 'left',func = river)
#just for testing with a Dirichlet on the right to see when "v" are higher
set.BC.fixedstate(model = Fmodel,where = 'right',value = "iso_h" )
...
1. The convective part of the CDE requires a 'velocity' which comes directly from the flow model, simply being the internal flux in the `Fmodel` and can be retrieved by `q_D = dataframe.internalfluxes(Fmodel)`.  
...
The solute transport model will simulate the movement of the pollutione in the groundwater which comes from a continuous source. The source is simulated with a prescribed amount of mass "loaded" into the groundwater which is called "mass loading".  
...
## Pollution for only 2 years

The current Cmodel is based on a stationary situation. This means the all aspects, including the massloading are constant in time. In reality pollutions are normally noticed much earlier.
To simulate the development of the pollution over time, a transient model is required.
...
```{r, animation.hook='gifski',fig.show='animate'}
# make a model copy of the stationary solute transport model
C2ymodel = copy.model(Cmodel)
#remove the concentration by initializing the model
do.initialize(C2ymodel,init = 0.0)
prev.C = state.fun(C2ymodel)
add.spatialflux(C2ymodel,C.sto,name = "mass_storage")

start_pollutant = 0.0
stop_pollutant = 365
delta.t = 10 #days for now
current.time= start_pollutant

#adding a plot to see the plume development
...
while(current.time < stop_pollutant)
{
  prev.C = state.fun(C2ymodel)
  solve.steps(C2ymodel)
  plot(nodes,prev.C(nodes),type = 'o',col='red',ylab = "concentration kg/m3",
       xlab= "distance (m)", main = "Transient solute concentration distribution",
       ylim = c(0,1))
  
  # lines(dataframe.states(C2ymodel)$x,dataframe.states(C2ymodel)$state,col = heat.colors(10)[current.time/10])
...
4.    Calculate all derivatives of the output (model output) w.r.t. parameters
5.    Calculate the variances of all parameters w.r.t. output
...
```{r eval = FALSE}
scale = list(fix.H = 0.1,
             k = 0.1*base$k, 
             p_t.rate = 0.1*base$p_t.rate,
             precip = 0.1*base$precip,
             bot.slope = 0.1*base$bot.slope, 
...
# ****EXTRA****
# Global Sensitivity Analysis (GSA) using SOBOL

The Sobol method is a global sensitivity analysis method which
decomposes the variance of the output of a model into fractions which can be attributed to the input factors.


## Setup of the Sobol analysis

The Sobol analysis requires a bit more work than the LSA.
The Sobol analysis requires a number of runs of the model, each time with a different combination of the parameters.
The Sobol analysis is based on the variance of the output of the model, which is decomposed into fractions which can be attributed to the input factors.

The same parameters and outputs will be used for the GSA.
However, some additional values are required for the Sobol analysis:


*  The number of runs of the model, $N$
*  The range of the parameters
    * as a first guess the range could be set to 2 time the scale of variation below and above the average/estimated value  
    
It is convenient to use the following structure. Using this structure means that very easily the analysis can be carried with the aid of the **GSOBOL** package developed by former lecturer and colleague Paul Torfs. 
This package depends on the following two packages: **lhs** and **truncnorm**. These packages can be installed from CRAN and need to be installed prior to GSOBOL.

```{r}
library(GSOBOL)
parnames = names(base) # this creates a vector with the names of the parameters
parmeans = unlist(base) # this creates a vector with the means of the parameters
parsd = abs(unlist(scale)) # this creates a vector with the standard deviations of the parameters
parlower = parmeans - 2*abs(parsd) # this creates a vector with the lower bounds of the parameters
parupper = parmeans + 2*abs(parsd) # this creates a vector with the upper bounds of the parameters
samplesize = 100
set.seed(234)  # setting a seed to see if this drastically changes the results
parsample = GaussianLHS(samplesize,mean = parmeans, sd = parsd, varnames = parnames, lower = parlower, upper = parupper)

```

### Inspecting the sample data

quickly inspect the sample data by looking at the first few rows and plotting the data.
<div class="answer">
...
hist(parsample[,fix.H],main = "Histogram of the sample",xlab = "fix.H",col = "lightblue",prob = T)
rug(parsample[,fix.H],col = "red")
lines(density(parsample[,fix.H]),col = "red")
hist(parsample[,k],main = "Histogram of the sample",xlab = "k",col = "lightblue",prob = T)
rug(parsample[,k],col = "red")
lines(density(parsample[,k]),col = "red")
hist(parsample$p_t.rate,main = "Histogram of the sample",xlab = "p_t.rate",col = "lightblue",prob = T)
rug(parsample$p_t.rate,col = "red")
lines(density(parsample$p_t.rate),col = "red")
hist(parsample$precip,main = "Histogram of the sample",xlab = "precip",col = "lightblue",prob = T)
rug(parsample$precip,col = "red")
lines(density(parsample$precip),col = "red")
hist(parsample$bot.slope,main = "Histogram of the sample",xlab = "bot.slope",col = "lightblue",prob = T)
rug(parsample$bot.slope,col = "red")
lines(density(parsample$bot.slope),col = "red")
hist(parsample$M.load,main = "Histogram of the sample",xlab = "M.load",col = "lightblue",prob = T)
rug(parsample$M.load,col = "red")
lines(density(parsample$M.load),col = "red")
hist(parsample[,Disp],main = "Histogram of the sample",xlab = "Disp",col = "lightblue",prob = T)
rug(parsample[,Disp],col = "red")
lines(density(parsample[,Disp]),col = "red")
hist(parsample$por,main = "Histogram of the sample",xlab = "por",col = "lightblue",prob = T)
rug(parsample$por,col = "red")
lines(density(parsample$por),col = "red")

par(old.par)

```
### Creating a function to run the model-train (flow and transport) for each sample

Since both model need to be run for each sample, it is convenient to create a loop calculating flow and transport and iterating through the `parsample` set.

```{r}
Mresult.data = c()  #container for storing the max. C data
Mresult.pump.data = c()  #container for storing the mass extracted by the pump&treat data
for (i in 1:nrow(parsample))
{
  fix.H = parsample[i,1]
  k = parsample[i,2]
  p_t.rate = parsample[i,3]
  precip = parsample[i,4]
  bot.slope = parsample[i,5]
  M.load = parsample[i,6]
  Disp = parsample[i,7]
  por = parsample[i,8]
  ...
  max_delta_t = delta.x/abs(v)
  delta.t = floor(min(max_delta_t))
  print(paste("current run nr: " ,i, " max. time step: ",delta.t))
  ...
  Mresult.data = c(Mresult.data,MRESULT())
  Mresult.pump.data = c(Mresult.pump.data,MRESULT_pump())
}
```

### adding the MRESULT data to the `parsample` data frame

The output from the  model(s) can now easily be analyzed through the function `Sobol()`.
See the help of the **GSOBOL** package for more information on the function `Sobol()`.

```{r}
output.ensemble = cbind(Modresult=Mresult.data,parsample)
output.ensemble.pump = cbind(Modresult=Mresult.pump.data,parsample)
```

### Some basic statistical analysis on the calculated output.

We now can have a look at some basic statistics of the calculated output, like the average, the standard deviation, the minimum and the maximum, the Coefficient of Variance and plot a histogram of the calculated output.

#### For the max. concentration at the end of the transient simulation

```{r}
summary(output.ensemble$Modresult)
hist(output.ensemble$Modresult)
rug(output.ensemble$Modresult,col = "red")
lines(density(output.ensemble$Modresult),col = "red")
Modresult.sd = sd(output.ensemble$Modresult)
Modresult.mean = mean(output.ensemble$Modresult)
print(paste("The standard deviation of the model output is: ",Modresult.sd))
print(paste("The mean of the model output is: ",Modresult.mean))
#boxplot(scale(parsample))
#coefficient of variation
CV = sapply(parsample,sd)/colMeans(parsample)*100
barplot(CV,main = "Coefficient of variation of the prediction",xlab = "parameters",ylab = "CV (%)")
```

### Running the Sobol analysis for the max. concentration at the end of the transient simulation

The Sobol analysis can now be run with the function `Sobol()`.
Here we wil have a look at the first order decomposition of the variance of the output.

```{r}
Sob1 = Sobol(output.ensemble)
pie(Sob1$var)
kable(Sob1$var, format = "markdown",main = "First order decomposition of the variance of the output")
print(paste("The sum of the variances is,: ",sum(Sob1$var)))

```
Now we  will do the decompostion of the second order as well, indicating interactions between the parameters.

```{r}
Sob2 = Sobol(output.ensemble,order = 2)
pie(Sob2$var)
kable(Sob2$var, format = "markdown")
```

### Visualizing the 'fit' of the parameters w.r.t. model output

Looking at the results of the Sobol analysis, we can see that the parameters `fix.H`, `k`, `p_t.rate`, `precip`, `bot.slope`, `M.load`, `Disp` and `por` have a different impact on the output of the model. The sum of the first order and second order variances is equal to the total variance of the output of the model. The first order variances indicate the impact of the parameters on the output of the model, while the second order variances indicate the interactions between the parameters.

```{r}
condexp.fix.H =  condexp1(Modresult,fix.H,data = output.ensemble)
condexp.k =  condexp1(Modresult,k,data = output.ensemble)
condexp.p_t.rate =  condexp1(Modresult,p_t.rate,data = output.ensemble)
condexp.precip =  condexp1(Modresult,precip,data = output.ensemble)
condexp.bot.slope =  condexp1(Modresult,bot.slope,data = output.ensemble)
condexp.M.load =  condexp1(Modresult,M.load,data = output.ensemble)
condexp.Disp =  condexp1(Modresult,Disp,data = output.ensemble)
condexp.por =  condexp1(Modresult,por,data = output.ensemble)
old.par = par(no.readonly = T)
par(mfrow = c(2,4))
par(mar=c(5,5,1,1))

with(output.ensemble, {
plot(fix.H,Modresult)
points(fix.H,condexp.fix.H$predictions,col = "red")
plot(k,Modresult)
points(k,condexp.k$predictions,col = "red")
plot(p_t.rate,Modresult)
points(p_t.rate,condexp.p_t.rate$predictions,col = "red")
plot(precip,Modresult)
points(precip,condexp.precip$predictions,col = "red")
plot(bot.slope,Modresult)
points(bot.slope,condexp.bot.slope$predictions,col = "red")
plot(M.load,Modresult)
points(M.load,condexp.M.load$predictions,col = "red")
plot(Disp,Modresult)
points(Disp,condexp.Disp$predictions,col = "red")
plot(por,Modresult)
points(por,condexp.por$predictions,col = "red")
})


```

#### For the mass extracted by the pump&treat

```{r}
summary(output.ensemble.pump$Modresult)
hist(output.ensemble.pump$Modresult)
rug(output.ensemble.pump$Modresult,col = "red")
lines(density(output.ensemble.pump$Modresult),col = "red")
Modresult.pump.sd = sd(output.ensemble.pump$Modresult)
Modresult.pump.mean = mean(output.ensemble.pump$Modresult)
print(paste("The standard deviation of the model output (mass extracted) is: ",Modresult.pump.sd))
print(paste("The mean of the model output (mass extracted) is: ",Modresult.pump.mean))

```

### Running the Sobol analysis for the mass extracted by the extraction well

```{r}
Sob1.pump = Sobol(output.ensemble.pump)
pie(Sob1.pump$var)
kable(Sob1.pump$var, format = "markdown",main = "First order decomposition of the variance of the output")
print(paste("The sum of the variances is,: ",sum(Sob1.pump$var)))
...
```{r}
Sob2.pump = Sobol(output.ensemble.pump,order = 2)
pie(Sob2.pump$var)
kable(Sob2.pump$var, format = "markdown")
```
...
```{r}
condexp.fix.H =  condexp1(Modresult,fix.H,data = output.ensemble.pump)
condexp.k =  condexp1(Modresult,k,data = output.ensemble.pump)
condexp.p_t.rate =  condexp1(Modresult,p_t.rate,data = output.ensemble.pump)
condexp.precip =  condexp1(Modresult,precip,data = output.ensemble.pump)
condexp.bot.slope =  condexp1(Modresult,bot.slope,data = output.ensemble.pump)
condexp.M.load =  condexp1(Modresult,M.load,data = output.ensemble.pump)
condexp.Disp =  condexp1(Modresult,Disp,data = output.ensemble.pump)
condexp.por =  condexp1(Modresult,por,data = output.ensemble.pump)
old.par = par(no.readonly = T)
par(mfrow = c(2,4))
par(mar=c(5,5,1,1))

with(output.ensemble.pump, {
plot(fix.H,Modresult)
points(fix.H,condexp.fix.H$predictions,col = "red")
plot(k,Modresult)
points(k,condexp.k$predictions,col = "red")
