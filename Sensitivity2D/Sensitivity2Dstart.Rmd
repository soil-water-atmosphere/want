...
IJsselmodel = copy.model(model = model)
...
<div class="exercise">
9.In the following sections 3.3 to 3.9, the relative contributions of the parameters to the uncertainty in the groundwater levels is calculated. The first example for Qrch is already given; finish the analysis for the other parameters. In section 3.10, add a discussion of how the contributions of all the parameters are distributed over the domain.
</div>
...
<div class="exercise">
10.In section 4.1 and 4.2, recalculate the local sensitivities of the groundwater levels of the model to the parameters (the way you did in section 2.1-2.7). In section 4.1 you should create a new base run which includes the new river bed resistance. Thereafter in section 4.2, recalculate the local sensitivities of the groundwater levels of the model. There is no need to make all the local sensitivity plots again.
...
Msample = load("AVS_mpsample.Rdata")
```
</div>
