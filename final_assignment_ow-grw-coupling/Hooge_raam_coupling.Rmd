...
Generieke opmerkingen: ik vind het wat verwarrend dat HR.xxx wordt gebruikt voor zowel (vaste) waarden als functies. Wellicht voor functies HR.fun.xxx gebruiken? 
GB. `HR.XXX` is bedoeld als onderscheid wat bij open water en wat bij grondwater hoort maar ik begrijp je punt.
Heb nu geen tijd meer om dat goed aan te passen. Voor 2022-2023 wellicht..
</div>
...
The Hooge Raam is located just below the small town Grave and is part of the waterboard "Aa en Maas". The surrounding area is mainly used as grasslands, some cornfields and a few forest plots. The area is reasonably flat where surface levels range from 15.5 till about 17.5 m AMSL (Above Mean Sea Level).
...
The length of this reach of the Hooge Raam is 1470 m. Upstream bottom level 14.50 m AMSL, downstream bottom level 11.80 m AMSL, bottom width 2.10 m, side slope 1.5 m/m.

Weir: crest width 2.0 m, crest height: 11.45 and 12.60 m.

Influx stationary upstream based on base flow as discharge of an upstream area of 250 ha (discharge 0.5 ? - 1 l/s/ha).
...
One of the reasons that this wetted cross section is so important is because of the important formula $Q = v A$, were $Q$ is the discharge, $v$ is the velocity and $A$ is the wetted cross section. As can be seen in the function above that the wetted cross section is dependent on the water depth $a$. One way to see the influence of the wetted cross section on the discharge is to investigate the relationship between $Q$ and $a$ for fixed velocities. Here is a typical example (it is customary to plot water depths on the vertical axis):
...
legend("bottomright", inset=.05, title="v (m/s)",
       c("1.25"), col=c("brown"),lwd=3,horiz=TRUE)
grid(col="black")
```
...
<div class="answer">
Reasonable velocities would be e.g. 0.25, 0.5, 0.75 and 1 m/d  
...
lines(Q1range,arange,lwd=3,col="green") #--
legend("bottomright", inset=.05, title="v (m/s)", #--
       c("0.25","0.5","0.75","1"), col=c("green","red","blue","brown"),lwd=3,horiz=TRUE) #--
grid(col="black") #--
```
</div>
...
<div class="answer">
-   The quotes makes that whenever the variable HR.Qin changes its value #-- the upstream boundary condition is adjusted to this new value. #--
</div>
<div class="question">
...
When surface runoff occurs, the state exceeds the surface level of 16.0 $m$ AMSL, a vertical resistance of the upper soil of 5 days can be assumed.
...
In de tekst hierboven stond een entrance resistance voor de ditch van 5 d, voor de surface van 3 d. In de code hieronder staat dit precies andersom. Ik heb daarom de tekst hierboven aangepast, zodat de antwoorden niet veranderen.
GB: Dank.
...
C_surface = 5 #d surface vertical resistance
...
while (current.time < end.time)
{
  ...
  plot(GW.Q.trans$time,GW.Q.trans$surface_runoff,ylim = Q.range, type = "o", col = "red",lwd =2,
       ylab = "flux rates in m2/d", xlab = "time in days")
  lines(GW.Q.trans$time,GW.Q.trans$storage_2flow,col="yellow",lwd = 2)
  ...
  legend("topright", inset=.05,
       legend=c("runoff","storage_2flow","storage_2storage","precipitation","drainage"), col=c("red","yellow","orange","blue","green"),
       lwd=3,horiz=FALSE)
#first 2 days 2*24 = 48 row
  plot(GW.Q.trans$time[1:48],GW.Q.trans$surface_runoff[1:48],ylim = Q.range, type = "o", col = "red",lwd =2,
       ylab = "flux rates in m2/d", xlab = "time in days")
  lines(GW.Q.trans$time[1:48],GW.Q.trans$storage_2flow[1:48],col="yellow",lwd = 2)
...
A "loose" coupling will be analyzed. With this the open water model will receive drainage and surface runoff fluxes from the groundwater model.  
...
The groundwater drainage and surface runoff are based on one parcel having a ditch distance of 125 m. These discharge rates have unit $m^2/d$ and need to be transformed to the total discharging area of the Hooge Raam into units of $m^3/s$. You can imagine all parcels, with an average ditch distance of 125m, were set behind each other resulting in a certain extent/length: $\frac {\text{total_drained_area}}{\text{ditch_distance}}$.
...
m2.day.to.m3.day klinkt alsof 'deel 1' (m2.day) vermenigvuldigd moet worden met een lengtemaat, maar in plaats daarvan wordt m2 gedeeld door m (geeft dus unit m). Duurde even voordat ik doorhad dat m2.day.to.m3.day ging om een vermenigvuldigingsfactor met unit m, welke *gebruikt* wordt om van m2.d naar m3.d te komen. Wellicht naam veranderen? (al heb ik nog geen goed alternatief bedacht)
GB: is inderdaad tricky. Heb dit maar gegeven..
</div>
...
m2.day.to.m3.day =   HR.discharge.area/L #from m2/day to m3/day; a length based on the total drained area divided by the L(ditch distance)
...
day2sec = 86400
m2.day.to.m3.day = HR.discharge.area/L
...

{
  ...
  HR.Q.trans = rbind(HR.Q.trans, c(current.time,wb[4,2],wb[2,2],wb[3,2],wb[4,3]))
...
For both the transient groundwater and the pseudo-stationary open water model the total volums of water during the simulation time are compared with both models.
...
Total_{out} = \left \{ \sum Qsto(t)+\sum Runoff(t) + \sum Drainage(t) \right \} \Delta t
$$
...
knitr::kable(volume.bal.GW,format = "simple")
...
$$
Total_{in} = \sum boundary_{upstream}\Delta t + \sum runoff \Delta t + \sum drainage \Delta t\\
Total_{out} = \sum boundary_{downstream}
$$
...
knitr::kable(volume.bal.HR,format.arg = list(digits = 8,nsmall = 3)) #,digits = 15)
...
```{r}
plot(GW.Q.trans$time, P.pattern(GW.Q.trans$time), col = "blue", lwd =2, ylab = "m/d",xlab = "days",type= "o", main = "Transient fluxes in mm/d", sub = "Total simulation time")
...
plot(plot.time, P.pattern(plot.time), col = "blue", lwd =2, ylab = "m/d",xlab = "days",type= "o", main = "Transient fluxes in m/d", sub = "First two days simulation time")
HR.discharge.m.d = HR.Q.trans$boundary_downstream * 86400 / HR.discharge.area
lines(HR.Q.trans$time,HR.discharge.m.d,col = "red", lwd =2)
HR.runoff.m.d = HR.Q.trans$runoff * 86400 / HR.discharge.area
lines(HR.Q.trans$time,HR.runoff.m.d, col = "brown",lwd =  2)
...
The max. precipitation is at day `r time.max.precip` and the max. discharge is at day : `r time.max.discharge`

The moment when the maximum discharge at the weir of the Hooge Raam is reached coincides with the max. runoff and drainage.
Why?
...
1.  Choose in total 5 or 6 parameters, preferably from both models, to carry out the sensitivity analysis with
2.  Estimate the scale of variation of the 5 or 6 parameters, based on your experience or what you have heard during courses
...
2.  `HR.So` the bed slope of the Hooge Raam
...
<div class="answer">
```{r}
#base = list(HR.n = HR.n, HR.So = HR.So, HR.Qin = HR.Qin, H_surface=H_surface, H_drainage = H_drainage)
#base = list(HR.n = HR.n, HR.So = HR.So, HR.Qin = HR.Qin, H_surface=H_surface, GW.S = GW.S)
base = list(HR.n = HR.n, HR.So = HR.So, GW.kD= GW.kD, H_surface=H_surface, H_drainage = H_drainage, GW.S = GW.S)
str(base)
```
</div>
...
scale = list(HR.n = diff(range.HR.n)/4,
             HR.So = diff(range.HR.So)/4,
             #HR.Qin = diff(range.HR.Qin)/4,
...
The local sensitivities can now be determined using a sequence of:  
...
<div class="question">
Set up the sensitivity sequence, see above, to determine the local sensitivities  
...
HR.So = base$HR.So*(1 + eps)
Sensitivity.loop()
dMdHR.So = (MRESULT() - M_base)/(eps*base$HR.So)
HR.So = base$HR.So
...
varM.HR.So = (scale$HR.So*dMdHR.So)^2
...
varM.total = varM.HR.n + varM.HR.So + varM.GW.kD + varM.H.surface + varM.H.drainage + varM.GW.S

relvarM = list(HR.n = varM.HR.n/varM.total,
               HR.So = varM.HR.So/varM.total,
...
pie(as.numeric(relvarM),labels = names(relvarM),
    main = paste("Standard deviation of M = ",round(sqrt(varM.total),4),"proper unit"),
