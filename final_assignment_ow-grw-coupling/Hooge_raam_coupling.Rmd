...
A storm event, a heavy rain shower, is the forcing for which we will investigate flow from the surface into the soil, towards ditches and finally to the Hooge Raam to get discharged at the weir downstream.
...
This document is divided into four parts. The first part contains the explanation and assignments for the open water model, the second will go into the details of the groundwater model, the coupling of both models is described in the third part and finally the last part (4) contains the setup to determine predictive uncertainties of the "model-train".

# PART 1 the Open water model

## Open water equations

This part of the document gives an introduction to the hydraulic modeling of the Hooge Raam river.

An attempt is made to make the document self contained. For that reason we start by giving some basic formulas, and more formulas will follow further in the document. This document should however not to be considered as a course in hydrodynamics. The formulas are cited to form the base of the code.
...
The weir at the downstream end of the Hooge Raam has the following properties:
...
The cross section can in general have a complex and spatially varying form. Here we take just a simple trapezoidal profile with a constant bottom width $b$ and constant side slope $m$:
...
Sometimes the question is posed the other way around. Assume that one wants to discharge a constant intensity of 10 $mm/day$ of an area of the size of 30 $km^2$ through this river.
...
The amount of water flowing over a weir is a function of the water height above the crest. For a rectangular weir this is a well known formula: $$Q_\mathrm{weir} = 1.83\; B_\mathrm{weir}\; (a-c_\mathrm{weir})^{3/2}$$ where $B_\mathrm{weir}$ is the width of the weir and $c_\mathrm{weir}$ is the height of the crest above the bottom of the river.
...
<div class="question">
Use the help of R or simply try different values for x in sign(x) to understand the sign function.
</div>
...
A proper value for this number will come from the groundwater model (see STEP 4 of this document). For an example value here we just take 50% of the inflow upstream as a total, that has to be distributed over the length of the river.
...
Although the addition of the drainage flux seems to have limited effect on the water level, one should however realize that the drainage flux is 50% of the influx.
...
$$
Q_{gw} = -kD * \frac {\partial H}{\partial x}
$$
with $Q_{gw}$ the groundwater discharge, $k$ the hydraulic conductivity, $D$ the thickness of the aquifer and $H$ the head.  

## Cauchy type of boundary condition to simulate the ditch drainage

$$
Q_{drainage} = \frac{H_{ditch} - H}{C_{ditch}}
$$
with $H_{ditch}$ the average ditch level and $C_{ditch}$ the entrance resistance of the ditches.
...
$$
Q_{runoff}= \frac{H_{surface} - H}{C_{surface}}
$$
with $H_{surface}$ the average surface level area and $C_{surface}$ the vertical resistance of the upper soil.


## Dimensions and parameters groundwater model

The average ditch distance is 125 m and the transmissivity ($kD$) in this area is estimated on 28 $m^2/d$ based on the hydraulic conductivity of the soil and the thickness.
...
These XXXX and YYYY only indicate that you have to fill in the correct values. 
...
The storage coefficient is estimated on 5%. For the storm events, the time step is set to 1 hour and the total simulation time is 10 days, starting with a stationary situation, followed by a storm event of a gradually increasing precipitation rate till 36 cm/hour and gradually decreasing again during 24 hours.  
...
```
The term <code>storage_2flow</code> stands for storage which is released to flow and <code>flow_2storage</code> is the amount of water which will be stored.
</div>
...
The groundwater drainage and surface runoff fluxes are based on one typical "land parcel" enclosed by an average ditch distance of 125 m. 
There are many of these parcels, surrounded by ditches, in the catchment area of the Hooge Raam. 
The illustration below shows a hypothetical allocation of parcels and ditches surrounding them
![illustration to indicate how the diffuse length is determined](Hooge_raam_diffuse_length.png) 

The discharge of precipitation excess rates from these parcels/fields towards the ditches is calculated by the 1D groundwater model and has the unit $m^2/d$. This needs to be transformed to the total discharging area of the Hooge Raam into units of $m^3/s$ by multiplying the total ditch length surrounding the parcels in the Hooge Raam catchment.For this, one could actually digitize all ditches from a map of this area but here we use the simple generic method of calculating the area of a rectangle: $Area = Length *Width$."Length" is here the average ditch distance between parcels (here 125 m) and "Width" the distance or range you would have if you would put all parcels end to end. 
Let's call this "Width" the *total diffuse ditch length* calculated as: $\frac {\text{total_drained_area}}{\text{average ditch_distance}}$ .
...
The task is to determine what the uncertainty of a model (predicted) result is due to the used parameter values in the model.

For example; suppose one wants to determine the uncertainty of the max head in a groundwater model, due to the model parameters (like kD, boundary-conditions, precipitation, for which you not have exact, or good values.

For this the following aspects need to be considered/determined:

1.  predictive result, like the max discharge (in time) of the Hooge Raam
...
To set up a proper procedure, have close look at the Local Sensitivity Analysis assignment 1, Local sensitivity 1D of last week.
...
In this example the predictive uncertainty will be the the max. weir discharge (downstream of the Hooge Raam).

There are several interesting model results for close inspection w.r.t. their 'predictive' uncertainties. To give some examples:
...
All results are stored in the following "containers" which are all simple data frames. Simply click on them in the upper right panel (Environmental tab of Rstudio) to what's all stored;
...
*  plot a pie chart to visualize the contributions to the predictive uncertainty 
