...
finally to the Hooge Raam to get discharged at the weir downstream.
...
This document is divided into four parts. The first part contains the
explanation and assignments for the open water model, the second will go
into the details of the groundwater model, the coupling of both models
is described in the third part and finally the last part (4) contains
the setup to determine predictive uncertainties of the "model-train".

# PART 1 the Open water model

## Open water equations

This part of the document gives an introduction to the hydraulic
modeling of the Hooge Raam river.
...
considered as a course in hydrodynamics. The formulas are cited to form
...
-   the flow in the Hooge Raam can be considered to be *stationary* (so
    the time derivatives in the equations above disappear)
...
<https://www.aaenmaas.nl/onswerk/regels/legger/> and open the 'Legger
oppervlaktewater'.

The following properties of the HOoge Raam are given in the table below:
...
| bottom width            | 2.1   | m      |
| side slope              | 1.5   | m/m    |
| bottom level upstream   | 14.50 | m AMSL |
| bottom level downstream | 11.80 | m AMSL |

The weir at the downstream end of the Hooge Raam has the following
properties: Weir: crest width 2.25 m, crest height: 1.00 m (w.r.t.
bottom elevation downstream).
...
library(FVFE1D) #load the library for the 1D finite elements/finite volumes models
library(nleqslv) #load the library for solving non-linear equations
library(gifski) #load the library for making gifs
```
...
the introduction this slope was called $S_o$. It is defined as the drop
of bottom level per unit length of the river. Here it is just a
constant:
...
form. Here we take just a simple trapezoidal profile with a constant
...
called the *water depth* and will be denoted by $a$. To make a plot of
the cross section we need to calculate the width for a whole range of
possible water depths.
...
plot(c(-rev(widthrange/2),widthrange/2),c(rev(arange),arange),
     xlab="y",ylab="a",main="cross section Hooge Raam",type="l",lwd=5,col="brown")
grid(col="black")
```

## Wetted area

One of the important cross section functions is the wetted area. In the
St-Venant equation denoted by $A$. For a trapezoidal cross section this
...
because of the important formula $Q = v A$, were $Q$ is the discharge,
$v$ is the velocity and $A$ is the wetted cross section. Some typical
(max) velocities depending of the soil type are given the table below
...
between $Q$ and $a$ for fixed velocities. Here is a typical example (it
is customary to plot water depths on the vertical axis):
...
But of course usually both and $A$ and $v$ change if the water depth $a$
changes.
...
wants to discharge a constant intensity of 10 $mm/day$ of an area of the
...
::: question
Repeat the calculation above for the rainfall intensities between 0.5 mm
and 15 mm and make a plot of the water depths vs these intensities.
...
plot(Is,aequis,type="l",col="blue",lwd=3,
     main = "precipitation (mm/d) vs. equilibrium water level",ylab="a equilibrium (m)",xlab ="precipitation (mm/d)")
grid(col="black")
```
:::
...
arrows(-HR.weir.width/2,HR.weir.crest,HR.weir.width/2,HR.weir.crest, length = 0.1, angle = 30, code = 3)
text(-0.5,HR.weir.crest+0.1,"crest width",pos=4, col = "blue")
grid(col="black")
arrows(0,0,0,1.0,length = 0.1, angle = 30, code = 3,col = "blue")
text(0.1,0.5,"crest height",pos=3,col="blue",srt=90)
```
...
where $B_\mathrm{weir}$ is the width of the weir and $c_\mathrm{weir}$
is the height of the crest above the bottom of the river.
...
plot(Qequirange,arange,main="Qequi and Qweir for HoogeRaam",
     xlab="Qequi (m3/s)",ylab="a (m)",lwd=3,col="blue",type="l")
lines(weirQrange,arange,lwd=3,col="red")
legend("bottomright", inset=.05,
       c("Qequiriver","Qweir"), col=c("blue","red"),lwd=3,horiz=FALSE)
grid(col="black")
...
legend("bottomleft", inset=.05,
       c("bottom","rough backwater"), col=c("brown","blue"),lwd=3,horiz=FALSE)
```
:::
...
curve, the *water level* with respect to AMSL (the $h$ in the plot
above) is the most common choice. These two quantities are connected
through the river, bottom height:\
$$
  h = z_b(x)+a 
$$
...
::: question
Use the help of R or simply try different values for x in sign(x) to
...
::: answer
-   The negative sign is there because water is leaving the system at
...
::: answer
-   The quotes indicate the variable string is defined and whenever the
    variable HR.Qin changes its value; the upstream boundary condition
    is adjusted to this new value. In de \</set.BC.fluxstate()/\>
    function above \</HR.weir.BC/\> is not enclosed by brackets because
    this refers to the function HR.weir.BC
:::
...
::: answer
-   Water depths should be larger than 10 cm
:::
...
trajectory. This is called *lateral inflow*. In terms of the FVFE1D
package this is called *spatialflux*. In this example we will think of
this flux coming from draining the arabel land where water collected in
ditches and drain tubes ending up in the river. We will think of this
...
A proper value for this number will come from the groundwater model (see
STEP 4 of this document). For an example value here we just take 50% of
the inflow upstream as a total, that has to be distributed over the
length of the river.
...
plot(nodes,HR.zb(nodes),col="brown",type="l",lwd=4,ylim=c(11,16),
     xlab="l (m)",ylab="h (m)",main=paste("backwater + lateral flow Hooge Raam"))
lines(HR.states.1,col="orange",lwd=3)
lines(HR.states.2,col="blue",lwd=3)
grid(col="black")
legend("bottomleft", inset=.05,
       legend=c("without lat. inflow","with lat. inflow"), col=c("orange","blue"),
       lwd=3,horiz=FALSE)
```

Although the addition of the drainage flux seems to have limited effect
on the water level, one should however realize that the drainage flux is
50% of the influx.
...
We will assume a 1D (1.5D) groundwater model simulating one specific
parcel, enclosed between two ditches or drains, within the drainage area
of this part of the Hooge Raam.\
...
$$
Q_{gw} = -kD * \frac {\partial H}{\partial x} \,\,\, [m^2/d]
$$ with $Q_{gw}$ the groundwater discharge, $k$ the hydraulic
conductivity, $D$ the thickness of the aquifer and $H$ the head.

## Cauchy type of boundary condition to simulate the ditch drainage

$$
q_{drainage} = \frac{H_{ditch} - H}{C_{ditch}} \,\,\, [m/d]
$$ with $H_{ditch}$ the average ditch level and $C_{ditch}$ the entrance
resistance of the ditches.

## Cauchy type op boundary condition to simulate surface runoff

$$
q_{runoff}= \frac{H_{surface} - H}{C_{surface}} \,\,\, [m/d]
$$ with $H_{surface}$ the average surface level area and $C_{surface}$
the vertical resistance of the upper soil.

## Dimensions and parameters groundwater model

The average ditch distance is 125 m and the transmissivity ($kD$) in
this area is estimated on 28 $m^2/d$ based on the hydraulic conductivity
of the soil and the thickness.
...
0.8 $mm/d$. For the transient model a situation of a storm event with a
maximum precipitation rate of 36 $mm/hour$ during one hour, will be used
as forcing.
...
::: question
Finish the functions in the chunk below (replace XXXX, YYYY). These XXXX
and YYYY only indicate that you have to fill in the correct values. It
does not mean that all XXXX or YYYY are the same.
...
The water balance for the transient groundwater model can be written as:
...
gradually increasing precipitation rate till 36 cm/hour and gradually
...
temporarily to determine the storage change. **Tip:** use the 'specific'
function in the FVFE1D package for for this.
...
while (current.time < end.time)
{
  ...
  colnames(GW.Q.trans) = c("time", "surface_runoff","storage_2flow","flow_2storage","precip","boundary")
  
  #plot some results
  Q.range = range(GW.Q.trans$surface_runoff,GW.Q.trans$storage_2flow,GW.Q.trans$flow_2storage,GW.Q.trans$precip,GW.Q.trans$boundary)
  ...
  lines(GW.Q.trans$time,GW.Q.trans$flow_2storage,col="orange",lwd = 2)
  lines(GW.Q.trans$time,GW.Q.trans$precip,col="blue", lwd = 2)
  lines(GW.Q.trans$time,GW.Q.trans$boundary,col="green")
  legend("topright", inset=.05,
       legend=c("runoff","storage_2flow","flow_2storage","precipitation","drainage"), col=c("red","yellow4","orange","blue","green"),
       lwd=3,horiz=FALSE)
#first 2 days 2*24 = 48 row
  ...
  lines(GW.Q.trans$time[1:48],GW.Q.trans$flow_2storage[1:48],col="orange",lwd = 2)
...
The term <code>storage_2flow</code> stands for storage which is released
to flow and <code>flow_2storage</code> is the amount of water which will
be stored.
:::
...
`HR.backwatermodel` and need to be "scaled" to proper dimensions;
$m^3/s$.\
The groundwater drainage and surface runoff fluxes are based on one
typical "land parcel" enclosed by an average ditch distance of 125 m.
There are many of these parcels, surrounded by ditches, in the catchment
area of the Hooge Raam. The illustration below shows a hypothetical
allocation of parcels and ditches surrounding them ![illustration to
indicate how the diffuse length is
determined](Hooge_raam_diffuse_length.png)

The discharge of precipitation excess rates from these parcels/fields
towards the ditches is calculated by the 1D groundwater model and has
the unit $m^2/d$. This needs to be transformed to the total discharging
area of the Hooge Raam into units of $m^3/s$ by multiplying the total
ditch length surrounding the parcels in the Hooge Raam catchment. For
this, one could actually digitize all ditches from a map of this area
but here we use the simple generic method of calculating the area of a
rectangle: $Area = Length *Width$."Length" is here the average ditch
distance between parcels (here 125 m) and "Width" the distance or range
you would have if you would put all parcels end to end. Let's call this
"Width" the *Total Diffuse Ditch Length* calculated as:
$\text{TDDL}=\frac {\text{total_drained_area}}{\text{average ditch_distance}}$
.
...
HR.discharge.area =  XXXXX#in m2
day2sec = 86400 #from day to seconds
TDDL =   HR.discharge.area/L #from m2/day to m3/day; a "total diffuse length" based on the total drained area divided by the L(ditch distance)

Q.drainage.m3.d = XXXXX#
Q.drainage.m3.s = XXXXX#
Q.drainage.m2.s = XXXXX#
...
day2sec = 86400
TDDL = HR.discharge.area/L #multiply with "total diffuse length"; i.e. Area/drain_distance

Q.drainage.m3.d = GW.Q.trans$boundary * TDDL
Q.drainage.m3.s = Q.drainage.m3.d/day2sec
Q.drainage.m2.s = Q.drainage.m3.s/HR.length

Q.runoff.m3.d = GW.Q.trans$surface_runoff * TDDL
...
`HR.backwatermodel` with drainage and surface fluxes coming from the
groundwater model.
:::
...

{
  ...
  plot(nodes,HR.zb(nodes),col="brown",type="l",lwd=4,ylim=c(11,18),
       xlab="l",ylab="h",main=paste("Transient water levels Hooge Raam, time = ",round(current.time,3)),
  lines(dataframe.states(HR.backwatermodel), col = "blue", lwd = 3))
  grid()
...
Total_{in} = \sum_{t= 0}^{t=T}Precip(t)Area\Delta t\\
...
in $m^2/d$. To come to volumes ($m^3$) one need to consider the "diffuse
length" and the time step.

```{r}
# Water volumes for the 1D groundwater model during the total simulation time

GW.volume.drainage = sum(GW.Q.trans$boundary) * TDDL * delt.t
GW.volume.runoff = sum(GW.Q.trans$surface_runoff) * TDDL * delt.t

GW.volume.storage_into = sum(GW.Q.trans$storage_2flow) * TDDL * delt.t
GW.volume.storage_outof = sum(GW.Q.trans$flow_2storage) * TDDL * delt.t

GW.volume.precipitation = sum(GW.Q.trans$precip) * TDDL * delt.t

GW.misfit = GW.volume.precipitation - GW.volume.drainage - GW.volume.runoff - GW.volume.storage_outof + GW.volume.storage_into

volume.bal.GW = data.frame(Groundwater = c("precipitation","storage_2flow","drainage","runoff","flow_2storage","total"),
...
Total_{out} = \sum_{t=0}^{t=T} boundary_{downstream}\Delta t
$$
...
plot(plot.time, P.pattern(plot.time), col = "blue", lwd =2, ylab = "m/d",xlab = "days",type= "l", main = "Transient fluxes open water model in m/d", sub = "First two days simulation time")
HR.discharge.m.d = HR.Q.trans$boundary_downstream * 86400 / HR.discharge.area
lines(HR.Q.trans$time,HR.discharge.m.d,col = "red", lwd =2)
HR.runoff.m.d = HR.Q.trans$runoff * 86400 / HR.discharge.area
lines(HR.Q.trans$time,HR.runoff.m.d, col = "brown",lwd =  2)
...
compared to the precipitation.\
The max. precipitation is at day `r time.max.precip` (blue vertical
line) and the max. discharge is at day : `r time.max.discharge` (red
vertical line)
...
three groundwater sinks in the same time step ($\Delta t$.

# PART 4 Sensitivity analysis

Here we will consider the local sensitivity analysis.

The task is to determine what the uncertainty of a model (predicted)
result is due to the used parameter values in the model.

For example; suppose one wants to determine the uncertainty of the max
...
1.  predictive result, like the max discharge (in time) of the Hooge
    Raam
...
`r HR.Q.trans$boundary_downstream[time.max.discharge]` in $m^3/s$ and
takes place at time : `r HR.Q.trans$time[time.max.discharge]` days.
...
1.  Choose at least 6 parameters, preferably from both models, to carry
    out the sensitivity analysis with
2.  Estimate the scale of variation of these parameters, based on your
    experience, what you have heard during courses or simply google it
3.  Create chunks to determine the local sensitivities
4.  Make use of the pre-programmed `sensitivity.loop()` executing time
    loops for each model and saving intermediate data
5.  Create a chunk to calculate the predictive uncertainties

This <code>sensitivity.loop()</code> contains many lines of code but
basically does the following:

1.  Run the transient groundwater model
2.  Store results, flux rates (m2/d) and heads, of the groundwater model
3.  Multiply the drainage and runoff flux rates with the diffuse length
    to come to m3/d and then to m3/s
4.  Run the transient open water model with the current drainage and
    runoff flux rates
...
Analysis assignment 1, Local sensitivity 1D of last week.

### Predictive uncertainty to analyse; the `MRESULT()`
...
In this example the predictive uncertainty will be the the max. weir
discharge (downstream of the Hooge Raam).

There are several interesting model results for close inspection w.r.t.
their 'predictive' uncertainties. To give some examples:
...
simple data frames. Simply click on them in the upper right panel
(Environmental tab of Rstudio) to what's all stored;

-   `GW.Q.trans` for all flux rates of the groundwater model in m2/d;
    each row is one time step
-   `GW.H.trans` for all heads in the groundwater model; each row is one
    time step
-   `HR.Q.trans` for all flux rates in the open water model in m3/s;
    each row is one time step
-   `HR.H.trans` for all heads in the open water model; each row is one
    time step

::: question
As an example, `MRESULT()` is the max. discharge of the Hooge Raam at
the weir.
...
Be aware that `MRESULT()` is based on the results which are contained in
the **current** data frames
`GW.Q.trans, QW.H.trans, HR.Q.trans, HR.H.trans`. The original value of
`MRESULT()` is assigned to `M_base`, in the code chunk of the following
section "Base run".
...
The chosen parameters will be altered by the `eps` value and reset after
the run.\
Next the local sensitivity of this parameter $P_i$ w.r.t. `MRESULT()`
will be determined; $\frac{\partial \text{MRESULT}}{\partial P_i}$.
...
water model loop in a sequence; the `sensitivity.loop()` function;
...
symbol/operator. It means that global `P`, `drainage`, `runoff`,
`GW.Q.trans GW.H.trans HR.Q.trans HR.h.trans` data.frames and the
`old.state` function are adjusted within this function avoiding a very
large set of arguments to pass to this function.
:::
...
**TIP**: you can use `eps` being an offset so, $P_i = P_i + eps$ or use
it as a fraction: $P_i = P_i*(1 + eps)$
...
-   variances for each parameter based on its scale of variance and the
    local sensitivity of that parameter w.r.t. `MRESULT()`
-   calculate the sum of these variances
-   calculate the relative contribution of each parameter to the total
    variance
-   plot a pie chart to visualize the contributions to the predictive
    uncertainty
-   a table of fractions, `knitr::kable()` is already implemented to
    inspect the procentual sensitivities

::: question
Determine the predictive uncertainty based on the aforementioned list
...
pie(as.numeric(relvarM),labels = paste(names(relvarM),round(unlist(relvarM)*100,1), "%", sep=" "),
    main = paste("MRESULT = ",round(MRESULT(),2),"m3/s , Standard deviation of MRESULT = ",round(sqrt(varM.total),2),"m3/s"),
