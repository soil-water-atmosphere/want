...
## Exercise 3
<span class="question">
3.1 What would be the mathematical notation of a 0-gradient condition (no flux) and fixed state ($s[5] = 1$) by using $s[4]$ and $s[5]$ and what would this look like in matrix notation (starting from the matrix defined for the Dirichlet boundary condition).
</span>

<span class="answer">
This quite a trivial question
$$
s[5] = 1\\
s[5] = s[4]\\
s[4] = 1
...
<span class="question">
3.2 Implement this matrix equation in the last chunk of section 5.2. 
</span>
...
# Plotting
plot(xcoor,s,ylab="state (m)",xlab="x (m) ",type='o',col='blue')
title(main='Advection Diffusion Equation, Cauchy boundary')
grid()
```
</span>