...
<span class='comment'> Something about approximation of ODEs in the time dimension? </span>

# Introduction
In this tutorial, you will analyse models that consist of ordinary differential equations that only depend on time. You will test the influence of parameter values and numerical schemes on the behavior of the solutions for simple 1D models, known as bucket or reservoir models (leaving out the spatial dimensions). This will be tackled with the help of Taylor series. 
...
Which simply means that the amount of water coming out of the bucket is proportional to the volume of water present inside the bucket.

The exact mathematical definition of a time derivative of the function $s(t)$ is as follows
...
Note that this solution is no longer exact. The difference between $t$ and $\Delta t$ is *finite* and has an associated error (we will come back to this later).
...
<span class="question">
1.1.2 Derive the exact solution of ordinary differential equation (ODE) of the emptying bucket (equation 1). 
</span>
...
<span class="question">
1.2.2 Run the following chunk to perform the actual simulation and visualise it.
```{r chunk3}
...
<span class="question">
1.2.3 Vary the `dt` parameter between 0.001 and 25. Are there any numerical problems arising beyond a certain value for `dt`? What are the problems occurring when choosing a value for `dt` close the high or low end of this range?
</span>
...
<span class="question">
1.2.5 The exact solution. Two options are available here; either unfold chunk4 and run the code or go to question 1 in the extended tutorial to add the exact solution to the previous plot yourself.
</span>
...
The former exercise can also be performed using more formal mathematics, such that the error of the numerical approximation can be calculated. For that we need Taylor series.

Taylor series (https://en.wikipedia.org/wiki/Taylor_series) can be used to make approximations of complex mathematical functions of the order that you desire. If you use an infinite series, the approximation is exact. The finite difference approximation that you have derived earlier, can also be derived using a Taylor series. TIP: if you are unfamiliar with this topic or you simply want a refresher you can watch the following video by 3Blue1Brown; https://www.youtube.com/watch?v=3d6DsjIBzJ4
...
<span class="question">6.3  Analyse the effect of the different reservoir parameters on the behavior of the model.</span>
