...
*maybe the hard track can this themselves*
...
<span class="comment">*insert graph that visualises all three plus real derivative*</span>
...
use the R-functions expression() and D() (derivative) and eval() to implement the first four terms of equation 13 incremently in chunk5 and plot this result on top of the existing plot. Note down the error at the first time step every time a term is added.
...
# Taylor series approximation
f = expression((-0.75 + 0.1875 * DT^(1)/2 - 0.046875 * DT^(2)/6) * DT + 3)   # constant by integration and evaluation (function D() and eval())
