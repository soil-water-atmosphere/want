...
2. Sensitivity analysis of variation through sampling:
The abbreviation __AVS__ will be used for this technique.
...
```
Technically: as the `grad.flux` function calls the global variables `kL`, `kR` and `D`, changing the values of these variables will immediately change the model to use this new values, without any extra code. This will prove to be productive in the sensitivity analysis. 
...
An increase in $D$ causes a decrease of the groundwater table over the whole domain, with the largest changes about in the middle. Again, this makes sense, because a deeper aquifer will allow easier discharge of groundwater. 
At the endpoints of the domain the sensitivity is zero as the state is determined by the boundary conditions.
The maximum groundwater level M also decreases, but its change is less than the changes in the middle of the domain.
...
At the endpoints of the domain, the sensitivity is zero as the state is determined by the boundary conditions.
The maximum groundwater level M also increases, but its change is less than in the middle of the domain.
...
We see that the largest scale of variation is at the boundaries: 10 cm.
This 10 cm was of course imposed by the modeller by choosing a scale of variation
for the boundary conditions of 10 cm. 
At no place in the domain the scale of uncertainty is less than 7 cm.
...
The creation of a parameter sample by means of a Latin Hypercube is the base of the AVS technique.
...
# 5. AVS variation analysis
The AVS variation analysis for the state at all nodes will take too much time and effort.
For that reason we will limit the AVS analysis to the data calculated by MRESULT.
