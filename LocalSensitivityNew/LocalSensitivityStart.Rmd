---
title: "Sensitivity practical 1 - Local sensitivity analysis"
...
# Learning goals
*  Performing local sensitivity analysis and analysis of variation through sampling on a simple model *(Exercise 1-3)*
...
<div class="exercise">
1. Read carefully the example of a sensitivity analysis, found in the 'Code Examples' folder. Reading through the html is a good recap of the material you should understand for the exam. If there
are questions about techniques or interpretations, please ask them.
</div>
...
The right hand side is a water divide: a zero flux boundary, which is automatically applied as the default in the FVFE1D package.
...
<div class="exercise">
3. Analyse the sensitivity of the model states and the MRESULT to all the parameters of the model. Perform both the local sensitivity analysis (LSA) and the analysis of variation through sampling (AVS).
...
- for all other cases: the boundaries should be invisible in the parameter histograms. 
</div>
...
<div class="answer">
See `LocalSensAnswer2.Rmd`.   
Some small changes are visible, for instance, there are changes in the minimum values of dstate/dkDl and dstate/dkDr. Changes in the sensitivity due to a different `eps` point to a nonlinear dependence of M on this parameter. The d{state}/d{hl} line over the model domain shows some fluctuations. These are numerical roundoff errors, and the value is still effectively 1 everywhere.
...
<div class="comment">
In question 4 and 5 the effect of eps is investigated, but we don't discuss what this means for how to choose eps. Maybe we should include something about that?
</div>

# 5. Influence of the random seed
<div class="exercise">
6. The sample generation for the AVS depends on the random seed. Do you see any significant differences in the AVS results if you change this seed?
</div>
...
<div class="answer">
See `LocalSensAnswer4.Rmd` (larger sample, seed=237) and `LocalSensAnswer5.Rmd` (larger sample, seed=238).   
With a sample size of about 1000, the differences between seed 237 and 238 are minimal. This shows that a sufficient sample size is important to ensure the AVS conclusions are correct. 
</div>
