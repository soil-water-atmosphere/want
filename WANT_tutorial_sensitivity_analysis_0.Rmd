...
<span class="question">
Plot the function $y = ax^6+bx$ for the domain $x = [0, 10]$ in the code chunk below. Base values for $a$ and $b$ are 3 and 7. Which parameter do you expect to be of more influence on y? 
</span>
...
<span class="question">
For the function $y = ax^3+bx$ for the domain $x = [0, 10]$, calculate the sensitivity of $y$ for $a$ and $b$ for 1) base values of 3 and 7 respectively, and 2) base values of 0.5 and 60 respectively. Does the base value influence the sensitivity? 
</span>
...
stepsize   = 0.1
par_sample = seq(0.1,20,stepsize) 
LSA_a      = matrix(nrow=length(par_sample),ncol=length(par_sample))
LSA_b      = matrix(nrow=length(par_sample),ncol=length(par_sample))
x = seq(0,10,1)
...
for (j in 1:length(par_sample)){
  for (i in 1:length(par_sample)){
        ...
        # calculate base y
        y_base = a_base*x^3+b_base*x
     
        # calculate perturbed y
        y_a    = (a_base+0.1)*x^3 + b_base*x

        # calculate SS
        SS_a   = sum((y_base-y_a)^2)

        # calculate LSA and store in vector
        LSA_a[i,j]   = SS_a / stepsize 
     }
}
plot(raster(LSA_a))
