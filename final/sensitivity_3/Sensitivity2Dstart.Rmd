...
In what follows we want to investigate the groundwater level in the housing area (which will be our "focus result" `MRESULT`. For that we need a node that falls within this area. Use the following plot to select a typical node in the housing area (zoom if needed).
```{r}
...
<div class="exercise">
9.In the following sections 3.3 to 3.9, the relative contributions of the parameters to the uncertainty in the groundwater levels is calculated. The first example for Qrch is already given. Finish the analysis for the other parameters.    
...
<div class="exercise">
13.Calculate the relative contribution of river.resist to the uncertainty in MRESULT and describe the changes. Plot the results in a pie chart.
</div>
...
## 5.1 AVS with a small sample size
Because the model takes a while to run, you will do a 'try-out' AVS with a (very) small sample size of 5.
...
## 5.2 AVS with a larger parameter set

The AVS has also been run for you with a sample size of 50:
```{r}
...
## 5.6 Discussion
<div class="exercise">
18.Finally, desribe your conclusions of the AVS analysis in section 5. Do the results differ from the local sensitivity analysis? What is the effect of the parameter sample size? Do you think the parameter set of 50 is large enough?
</div>
...
Looking at the relative contributions of the parameters, Qrch turns out to be dominant, just like before in the local sensitivity analysis. However, the kD now dominates over the river resistance and drainage resistance, while in the local sensitivity they had about equal contributions. This might be influenced by the small parameter sample size (50 is still not that much).
</div>

<div class="comment">
COMMENTS:
Ik heb de AVS nu opgelost door ze het zelf te laten doen met een kleine sample en dan de grote sample te geven.
Het knitten van het hele document duurt nog steeds wel even, maar ik denk niet dat er nog veel aan te doen is.
</div>
