...
where $Q_{st}$ is the storage change. In this 1-dimensional example, the volume step `dV` equals `dx`.

<div class="question">
5. What is the dimension of this flux and why is the storage change negatively related to the rate of change in the state variable (note the minus in the equation above)?
</div>
...
```{r, eval=FALSE}
#add.spatialflux(oneDtransientmodel, storage_change, name="storage")
add.spatialflux(oneDtransientmodel, rate=storage_change, name="storage")
```
</div>

Now that you know how the storage change can be added to the model, the function itself should be defined. We need to provide a function that calculates the storage change for a specific location (the nodes). The definition of the storage change should be a function that takes as arguments the location and the value of the state variable at this location. The return-value is the storage change.

To develop such a function, we first need to discretize the formulation of the storage change. Discretization of this function looks as follows: 

$$Q_{st}=-\int_{V}\frac{\theta_{t+\Delta t}-\theta_{t}}{\Delta t}dx$$

where $Q_{st}$ is the value of the storage change at the next time step (which we want to calculate), $\theta_{t}$ is the potential temperature at the current time step (which is generally known), $\theta_{t+\Delta t}$ is the potential temperature at the next time step and $\Delta t$ is the time step itself.

<div class="question">
8. Define the storage change function below, call it 'storage_change' and add the storage_change function to your transient model. Use a function called 'oldstate(x)' that retrieves the state values of the current time step which takes position as an argument.
</div>
...
<div class="answer">
8. The storage change depends on position and state. Even though oldstate(x) is not defined yet, we can already define the storage change function (but we are not able to run it yet, without defining the oldstate function). The oldstate-function returns the state of the system at a specific location, x, at the current time step. This information is needed in order to calculate the gradient of the state in time. dt still needs to be defined too. 
```{r}
storage_change = function(x, state)
{
...
<div class="question">
13. Adapt your transient model calculations using the tips below and create some plots of the intermediate results. At least make a plot of the flow over both boundaries, state at the extraction well and storage change change. Examine the plots critically, can you explain the intermediate plotting results?
...
In this last question a non constant recharge pattern will be applied to the non linear transient model, 'trans2model'. The recharge data that we will use comes from a meteorological station located near Eibergen in the catchment area of the brook Hupsel. The recharge data is shown in the code chunk below: 
