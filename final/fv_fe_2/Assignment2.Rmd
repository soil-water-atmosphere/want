...
The storage change $Q_{st}$ is directly related to the increase or decrease in the value of the state variable stored within the system. For heat storage in the atmosphere, it is given as:

$$Q_{st} = -\int_{V}\frac{\partial \theta}{\partial t}dV$$

where $Q_{st}$ is the storage flux. In this 1-dimensional example, the volume step `dV` equals `dx`.
...
<div class="answer">
The storage change has the same units as the internal and external flux and is prescribed in change in the state variable per time unit, multiplied with the length (as this is a 1D problem) over which it is integrated.
The storage flux is negatively related to the rate of change in the state variable, as this is the convention. While in the advection-diffusion equation the $\partial s/\partial t$ is on the left-hand side, in the solution of the water balance, we move it to the righthand side. The storage change then acts as a negative flux. It can be interpreted as the flux that needs to be added in order to compensate for the non-closure of the water balance.
</div>
...
$$Q_{st}=-\int_{V}\frac{\theta_{t+\Delta t}-\theta_{t}}{\Delta t}dx$$
...
<div class="question">
8. Define the storage flux function below, call it 'storageflux' and add the storageflux function to your transient model. Use a function called 'oldstate(x)' that retrieves the state values of the current time step which takes position as an argument.
</div>

<span class="comment">**[LM]** Chiel; Gegeven dat je gebruikt maakt van oldstate voor huidige state, is het dan niet logischer om in de vergelijking hierboven t en t-dt te gebruiken, in plaats van t en t+dt?

**[CvH]** Wellicht! Ik laat 'm even open...**[/CvH]**
</span>
...
{
   storageflux = - (state - oldstate(x)) / dt
   return(storageflux)
}
...
During time an amount of water can be stored into the soil or released from it. This storage change of water takes place in the pore space of the soil.  
Figure 2 illustrates water stored in the pore space of a soil column.  
![](water_storage.png)

Figure 2: Water stored in the soil. Above the groundwater table (the unsaturated zone) pore space is filled with air, below it (saturated zone) it is filled with water.  

In this assignment an extraction well is switched on and during time a _draw down cone_ of the head develops (see figure 1). In the beginning of the extraction most water is extracted from this pore space. As a consequence the head at the vicinity of the well will drop since the soil releases stored water from pore space and is then transported into the well tube.  Due to the head drop near the well, a head gradient develops creating groundwater flow towards the well.  
...
Finally, the effect of releasing water from storage towards the well, is in essence a decrease of $l$ and therefor head.


</div>


To account for this storage change an additional (flux) term need to be added to the model's balance:
...
1.    $q_{sto}$ Storage change being a recharge or discharge [$\mathsf{m/d}$]
2.    $S$ Storage Coefficient [$\mathsf{m^3/m^3}$]
3.    $H(x)_{new}$ Head at the new time step [$\mathsf{m}$]
4.    $H(x)_{old}$ Head at the previous (old) time step [$\mathsf{m}$]


<div class="question">
1.What are the units of the variables above and?
</div>
