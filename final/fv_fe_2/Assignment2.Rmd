---
title: "Assignment 2: Finite Volumes Transient Flow in 1D"
...
CheckInstallPackages(c("knitr","FVFE1D","gifski"))
```
...
<div class="answer">
The mathematical setup of the first model  
...
<div class="exercise">
Implement the numerical part of the model, and solve the model for the initial (here stationary) situation to obtain values of the state variables for all volumes (nodes).  Since the flux terms for these models are all relatively small (in numbers), it is wise to increase the convergence criteria.  This can simply be set as an extra argument of the ``solve.steps()`` function. Have a look at the help of this function and set e.g. ``MAMcrit = 1e-8``. Since there are more convergence criteria to consider, it is wise to set these even more strict (i.e. smaller values) to be sure that MAMcrit is met.
...
<button type="button" onclick="showHint(2002)">Show Hint</button>
<p id="Q2002"> </p>
...
<div class="answer">
The numerical setup of the first model  
...
<button type="button" onclick="showHint(2003)">Show Hint</button>
<p id="Q2003"> </p>
...
<div class="exercise">
What is the dimension of this external flux and why is the storage flux negatively related to the rate of change in the state variable?
</div>
...
<div class="answer">
The storage flux has the same units as other external fluxes and is prescribed as a change in the state variable per time unit, multiplied with the length (as this is a 1D problem) over which it is integrated.
...
<div class="exercise">
Make a new model which is an exact copy of your `oneDinit`-model. Call this copy `oneDtransientmodel`.
</div>

<button type="button" onclick="showHint(2004)">Show Hint</button>
<p id="Q2004"> </p>
...
<div class="exercise">
Check the documentation and try to find the function which should be used to add the storage change function to your transient model.
</div>
...
<div class="answer">
Add the storage change to your transient model with the function below. As long as you do not evaluate this code block in the knitting procedure, it is allowed to refer to arguments that are not defined yet, such as the storage change argument.
...
<div class="exercise">
Define the storage flux function below, call it `storage_flux` and introduce it to your transient model. Use a function called `oldstate(x)` that retrieves the state values of the current time step which takes position as an argument.
</div>

<button type="button" onclick="showHint(2005)">Show Hint</button>
<p id="Q2005"> </p>
...
## Setup of the transient simulation
...
<div class="exercise">
Include a line of code that tells the script that the starting time is zero (use the variable name ``time`` for time in your code).
</div>
...
<div class="exercise">
Include a piece of code that tells the script that the time should increase with $\Delta t$.
</div>
...
<div class="exercise">
Define the lower boundary condition as described by the cooling of the surface equation and add this transient boundary condition to your transient model.
</div>
...
<div class="exercise">
Add this piece of code and verify that it indeed gives the value of the state at the previous time step, $t=0$ in this case, for the locations $x=24.5$ and $x=50.5$.
</div>
...
<div class="exercise">
Add a piece of code that defines the numerical formulation (nodes, and discretization type) of your model. The nodal distance should be two meters. Thereafter, print a summary of your model to check whether the storage change was implemented correctly.
</div>
...
## One step transient model simulation
...
<div class="exercise">
Complete the last code chunk with ``solve.steps(oneDtransientmodel)`` to calculate the situation at $time=\Delta t$.
</div>

<div class="answer">
Here the output of solving the model (number of iterations, improvements etc) is writen to the variable `solve_1_dt`  
...
<div class="exercise">
Plot the profiles of the state variables and the internal fluxes for time $t=\Delta t$ and compare these to the profiles at $t=0$.
</div>
...
<div class="exercise">
Calculate the (energy) balance with  ``dataframe.balance(oneDtransientmodel)`` and inspect the balance terms of the whole model.
</div>
...
<div class="exercise">
Create a new chuck based on the pseudo-code below in order to make the transient version. Take the following actions and simulate the model for 4 hours.

* Make sure that the starting states of the transient model are set explicitly before the time loop, otherwise the starting states are set to zero.
* Add some arguments between the curly brackets on top of your R-code block to create an animation: ``{r, animation.hook='gifski',interval= 0.1}``) to create an animation. If the animation does not display in the markdown, open the html in your browser (Chrome, Edge..).
</div>

```
# Pseudo code for time loop.
time = 0
...
while (time < end.time)
{
   ...
   @create function containing the old state
}
```
...
<div class="exercise">
Run the code. If all went well, and the end time was chosen properly, you should see a movie of the state transitions between the initial situation and the situation after 4 hours.
</div>
...
<div class="exercise">
Use these ideas to make one plot showing the external flux at the lower boundary. Also, make one plot that both presents the state value at a location of 50 m above the surface and the state value at the lower boundary as a function of time.
</div>
...
* L = 200 $\left[\mathsf{m}\right]$

* Dirichlet boundary conditions left and right river = 5.0 $\left[\mathsf{m}\right]$

* kD = 45 $\left[\mathsf{m^2/d}\right]$
 for type 1 flow
* k = 10 $\left[\mathsf{m/d}\right]$
 for type 2 flow
* The nodal distance $\Delta$ x = 10 $\left[\mathsf{m}\right]$

* Recharge R = 0.001 $\left[\mathsf{m/d}\right]$

* Extraction rate of the well is 0.25 $\left[\mathsf{m^2/d}\right]$
 at 90 $\left[\mathsf{m}\right]$
  from the left
...
## Transient systems: The storage change  
...
<div class="exercise">
What are the units of of several variables this model and are the units for the different fluxes for the balance the same?
...
| System variable | Unit          | 
| ----------------|---------------| 
| State ($H$)     |   $\left[\mathsf{m}\right]$|
| Internal flux   | $\left[\mathsf{m^2/d}\right]$|
| External recharge flux   | $\left[\mathsf{m/d}\right]$|
| External well flux   | $\left[\mathsf{m^2/d}\right]$|
| Storage flux    | $\left[\mathsf{m/d}\right]$|
| Space (scale)   | $\left[\mathsf{m}\right]$|
| Time  (scale)   | $\left[\mathsf{d}\right]$|

The internal flux has unit $\left[\mathsf{m^2/d}\right]$ and for that the (water) balance as well. External and Storage flux now need to be intergrated over the volume length of the nodes.  

</div>

## Setting up the stationary part of the model  
...
<div class="exercise">
Now, create, solve and plot an identical stationary model with the extraction well at 90 m from the west. You may use copy.model() from the FVFE1D package to prevent repetitive code blocks. Call this copied model 'stat_well' and assign a new name to it using set.name().
</div>
...
<div class="exercise">
Examine the origin of the water extracted by the well. In other words, where does this extracted water come from? Give your answers as a fraction of the total amount of extracted water.
</div> 

<button type="button" onclick="showHint(2009)">Show Hint</button>
<p id="Q2009"> </p>
...
# The recharge distribution is homogeneous. Each cell, except both volumes at the boundary of the domain because these are half the size of the other volumes, captures an equal amount of water.
...
<div class="exercise">
Plot the head distributions of stat_no_well and stat_well in one graph by simply using 'dataframe.states()'.
</div>

<button type="button" onclick="showHint(2010)">Show Hint</button>
<p id="Q2010"> </p>
...
<div class="exercise">
Define the storage change and its dependencies. Use 0.15 $\left[\mathsf{-}\right]$
...
<div class="exercise">
To create a transient model the following has to be implemented: 

* Make a copy of stat_no_well and call it transientmodel
* Give this model a new name (this is not required, but it is recommended)
* Add the extraction well to this model 
* Add the storage change flux to the model 
...
Here we first will simulate a transient situation on just one time step.  With this it becomes clear that over time the state distribution changes from the stationary one without an extraction well to the new situation at time = $\Delta t$.  When looking at the balance of this model it becomes clear that a certain amount of flux comes from storage.
...
<div class="exercise">
When looking at the storage change w.r.t. the other balance terms, what can you conclude from it? 
</div>

<div class="answer">
A very large amount of the extracted water comes from storage.  
The outflow at the boundaries is very close to the stationary situation.
</div>

<div class="exercise">
Adjust the time step from 0.5 day to 100 day(s) and rerun the previous chunk
**Do not forget** to reset dt to 0.5 day aferwards.
</div>
...
<div class="answer">
With a small time step most of the flux comes from the storage change and with a large time step most of the water comes from recharge and the closest boundary.  
</div>

## Implementing the time loop
To cycle through the time steps $\Delta t$ of 0.5 $\left[\mathsf{d}\right]$
 starting at **begin.time** and finalizing at **end.time**, a time loop needs to be implemented. Use the pseudo-code of the meteorological exercise in Section 1.4.2 as a reference.

<div class="exercise">
Adapt the code for the specific case here. Take the following actions and simulate the model for 30 days and set `dt` ($\Delta t$) to 0.5 $\left[\mathsf{d}\right]$

* Note that the starting states of the transient model should be initiated explicitly, otherwise the starting states are set to zero. Initialize the transient model with the resulting states from `stat_no_well` with `do.initialize()`. By this initialization the spin-up time of the model is minimized. It should be set prior to the time loop.  
* add `animation.hook='gifski',interval= 0.1` between the curly brackets on top of your R-code block to create an animation.

You should see a "movie" of the state transitions during the execution of the transient model. A real movie can be included when one Knits the document. If the animation does not work, try opening the html in Google Chrome.
</div>

<button type="button" onclick="showHint(2011)">Show Hint</button>
<p id="Q2011"> </p>
...
<div class="exercise">
Adapt your transient model calculations using the tips below and create some plots of the intermediate results. At least make a plot of the flow over both boundaries, state at the extraction well and storage change. Examine the plots critically, can you explain the intermediate plotting results?

* Use **result.store = data.frame()** to have a container for the required data.
* Use **curresult = dataframe.states(transientmodel)** to store the heads.
* Use **curboundaries = dataframe.boundaries(transientmodel)** to store the outflow of both rivers.
* Use **wbal = dataframe.balance(transientmodel)** to obtain the intermediate storage change.
* Use **result.store = rbind(result.store, data.frame('t'=time, 'Hatwell'=curresult$state[9], 'Qoutleft'=curboundaries[1,4], Qoutright'=curboundaries[2,4], 'Qstorage'=wbal[2,2]))** to add the intermediate data to your container.
</div>
...
<div class="exercise">
Is the simulation time long enough for the model to reach stationary conditions?
</div>
...
<div class="answer">
In order to answer this question all three the plots that you created above can be used to argue that the model did not reach a steady state. The flow over both boundaries is not constant in time. There is still change in the head at the extraction well and the storage change is not zero yet. In the model the storage change is used as the criterium for being stationary. This will approach zero, but it will not become exactly zero. This mismatch is called the misfit of the stationary solution.
</div>

## **Extra assignment** Using the non-linear (type 2) internal flux
In the previous questions we have used an average transmissivity in the internal flux function which is defined as flow of type 1. Now, this type 1 flow will be replaced with type 2 flow. This means that the transmissivity is based on the saturated thickness at a specific location. The hydraulic conductivity, 10 $\left[\mathsf{m/d}\right]$
, is constant.

<div class="exercise">
Solve the new transient model and use the same approach as in the first transient model. Plot the intermediate result for comparison.
...
<button type="button" onclick="showHint(2012)">Show Hint</button>
<p id="Q2012"> </p>
...
<div class="exercise">
Compare the intermediate results of both transient models by graphs and numbers. How does the change from type 1 to type 2 flow change your model results?
</div>
