...
To develop such a function, we first need to rewrite the storage change in the form of an external flux:

$$Q_{st}=-\int_{V}\frac{\theta_{new}-\theta_{old}}{\Delta t}dx$$,

where $Q_{st}$ is the negative value of the storage change, $\theta_{old}$ is the potential temperature at the time step for which we have solved before (or the initial condition at the very first time step), whereas $\theta_{new}$ is the potential temperature at the time step for which we are solving. Parameter $\Delta t$ is the time step itself.

<div class="question">
8. Define the storage external flux function below, call it `storage_flux` and introduce it to your transient model. Use a function called `oldstate(x)` that retrieves the state values of the current time step which takes position as an argument.
</div>
