...
<div class="question">
17. Create a new chuck based on the pseudo-code below in order to make the transient version. Take the following actions and simulate the model for 4 hours.
...
<div class="question">
1. What are the units of of several variables this model and are the units for the different fluxes for the balance the same?
...
### Introduction
The models,  both the meteorological and hydrological one, consider transient flow in 1D. Such models are mathematically given by the requirement that for each element $[xL,xR]$ the mass balance holds:
$$
\begin{matrix}
Q_{storage} &=& Q_{internal_{in}}-Q_{internal_{out}}+Q_{external_{spatial}}+Q_{external_{point}}\\
\int_{x_L}^{x_R} S(x)\frac{\partial H}{\partial t}(x)\;dx &=& \overrightarrow{Q}(x_L)-\overrightarrow{Q}(x_R)+\int_{x_L}^{x_R} \hat{Q}(x)\;dx+ \sum_{p \in [x_L,x_R]} \dot{Q}(p)\\
\end{matrix}
$$
The model for this Theory Question concerns the last model; a section between two rivers, implemented as boundary conditions, where transient flow towards an extraction well is simulated.  
The numerical approximation used was the Finite Volume technique. Suppose the model was approximated with the Finite Element technique using the linear approach.


### Theory Question

Suppose the well is located at x=83 m. 

<div class="question">
Calculate (by hand) the contribution of the well rate to the neighboring nodes.  
</div>

You may check the results by simply set the discretization to `method = "FE"` of the basis stationary model (with well), `stat_well`.  

<div class="answer">
The well is a point flux; $\dot Q(p)$ and is redistributed with the aid of the base function $\mathcal{H}_{k}$ with $k$ indicating the nodes.  
For the left node $k$ of the element, the ratio is $\mathcal{H}_{k_L}\big(x(p)\big) = \frac{x_{k+1}-x(p)}{x_{k+1}-x_k}$.  
For the right node $k+1$: $\mathcal{H}_{k_R}\big(x(p)\big) = \frac{x(p) - x_{k}}{x_{k+1-x_k}}$

The well is located at $x=83$m , on the element between nodes $x_L = x_k=80$m and $x_R = x_{k+1}=90$m.  
$\dot Q(k)=\mathcal{H}_{k_L}\dot Q(83) = \frac{90-83}{90-80}0.25=\frac{7}{10}*0.25=0.175\;m^2/d$  
$\dot Q(k+1)=\mathcal{H}_{k_R}\dot Q(83) = \frac{83-80}{90-80}0.25=\frac{3}{10}*0.25=0.075\;m^2/d$  

```{r}
FE_stat_well = newFLOW1D(domain = domain, systemfluxfunction = flux.fun,name = "FE_stationary model")
set.BC.fixedstate(FE_stat_well,where = "left",value = left.bound )
set.BC.fixedstate(FE_stat_well,where = "right",value = right.bound)
add.spatialflux(FE_stat_well,rate = 0.001, name = "recharge")
add.pointflux(FE_stat_well,at = 83, value = -0.25,name = "Well")
set.discretisation(FE_stat_well,nodes=nodes,method = "FE")
solve.steps(FE_stat_well)
plot(FE_stat_well,fluxplot = T)
dataframe.balance(FE_stat_well,region = c(9))
dataframe.balance(FE_stat_well,region = c(10))
dataframe.externalfluxes(FE_stat_well)[9:10,]
```
