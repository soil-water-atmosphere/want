...
### Introduction
The transient groundwater flow model simulated the effect of an extraction on flow between two rivers. The numerical approximation used was the Finite volume technique.  
Suppose the model was approximated with the Finite Element technique using the linear approach.
The water balance would then be, for each element $[x_L,x_R]$:
$$
\begin{matrix}
Q_{storage} &=& Q_{internal_{in}}-Q_{internal_{out}}+Q_{well}+Q_{precipation}\\
...
<div class="answer">
The well is a point flux; $\dot Q(x)$ and is redistributed with the aid of the base function $\mathcal{H}_{k}$ with $k$ indicating the nodes.  
For the left node $k$ of the element, the ratio is $\mathcal{H}_{k_L} = \frac{x_{k+1}-x(p)}{x_{k+1}-x_k}$ .  
For the right node $k+1$: $\mathcal{H}_{k_R} = \frac{x(p) - x_{k}}{x_{k+1-x_k}}$ 
