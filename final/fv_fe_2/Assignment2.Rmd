...
<div class="content">
The potential temperature is defined as: $\theta=T\frac{P_0}{P}^{R/c_p}$ .  

Where $P_0, P, R$ are the reference and local pressures and the gas constant.  
$C_p$ is the specific heat capacity which is a property of an air parcel for storing or releasing heat (energy).  
...
The storage flux is negatively related to the rate of change in the state variable, as this is the convention. While in the advection-diffusion equation the $\partial s/\partial t$ is on the left-hand side, in the solution of the water balance, we move it to the right hand side. The storage change then acts as a negative flux. It can be interpreted as the flux that needs to be added in order to compensate for the non-closure of the water balance.
</div>
...
<div class="answer">
Add the storage flux to your transient model with the function below. As long as you do not evaluate this code block in the knitting procedure, it is allowed to refer to arguments that are not defined yet, such as the storage flux argument.
```{r, eval=FALSE}
#add.spatialflux(oneDtransientmodel, storageflux, name="storage")
add.spatialflux(oneDtransientmodel,rate=storageflux,name = "storage")
```
</div>
...
**[GB]** ik zou het vermijden door : $$Q_{st}=-\int_{V}\frac{\theta_{new}-\theta_{old}}{\Delta t}dx$$ te gebruiken
overigens laat ik in dezelfde vergelijking bij het hydrovoorbeeld zien that de state een functie van plaats en tijd is...
...
<div class="question">
13.Add a piece of code that defines the numerical formulation (nodes, and discretization type) of your model. The nodal distance should be two meters. Thereafter, print a summary of your model to check whether the storage flux was implemented correctly.
</div>
...
```
The storage flux is about 10% of the external flux 
</div>
...
To cycle through the time steps $\Delta t$ of 300 $\mathsf{sec}$ starting at **begin.time** and finalizing at **end.time**, a time loop needs to be implemented. A small chunk of pseudo code that describes how to do this is given in figure 3.  

<div align="center">
![](time_loop.png)  
  
  
Figure 3: Pseudo code for the time loop
...
<div class="question">
17. Use the pseudo-code and convert it to actual R-code. Take the following actions and simulate the model for 4 hours.
...
Finally, the effect of releasing water from storage towards the well, is in essence a decrease of a volume of water expressed as $l$ and therefor head.
...
<div class="question">
6.Define the storage flux and its dependencies. Use 0.15 $\left[\mathsf{-}\right]$
...
Here we first will simulate a transient situation one just one time step.  With this it becomes clear that during time the state distribution changes from the stationary one without an extraction well to the new situation at time = $\Delta t$.  When looking at the balance of this model it becomes clear the a certain amount of flux comes from storage.
...
To cycle through the time steps $\Delta t$ of 0.5 $d$ starting at **begin.time** and finalizing at **end.time**, a time loop needs to be implemented. A small chunk of pseudo code that describes how to do this, was given in figure 3 above of the meteorological part. 

<div class="question">
12. Use the pseudo code and convert it to actual R-code. Take the following actions and simulate the model for 30 days and reset dt ($\Delta t$)
0.5 $\left[\mathsf{d}\right]$
...
<div class="question">
13. Adapt your transient model calculations using the tips below and create some plots of the intermediate results. At least make a plot of the flow over both boundaries, state at the extraction well and storage flux change. Examine the plots critically, can you explain the intermediate plotting results?
...
<div class="answer">
```{r animation3, echo=FALSE, fig.show='animate', aniopts='controls', interval=0.1,eval=TRUE}
