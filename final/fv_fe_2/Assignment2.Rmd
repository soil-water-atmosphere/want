...
In this assignment transient systems will be introduced. The assignment consists of two different parts. We will be building a meteorological model in the first part and a hydrological model in the second part.
...
Before we can start studying the transient behaviour of the model, we have to create a stationary model first. The solution of this stationary model describes the situation right before the surface starts to cool. The states of this stationary model will be used as input for our transient model. Note, although the model is stationary, there is loss of energy from the model due to the external flux. The starting equilibrium situation incorporates this external flux.
...
<div class="question">
3.Implement the numerical part and solve the model for the initial situation to obtain values of the state variables for all volumes (nodes). Make plots of the state variable and the internal fluxes as a function of height. Switch the axes such that the heights are presented on the vertical axis. Thereafter, print a summary of the model. Take a look at the documentation of the plot function from the FVFE1D-package and consider the arguments of this function. For instance, you may use the "FVstyle" parameter to plot the actual values of the control volumes in which the states are constant (this is a consequence of the finite volumes method).
</div>
...
<div class="question">
6.Make a new model which is an exact copy of your oneDinitmodel. Call this copy oneDtransientmodel.
</div>
...
where $θ_{st}$ it the value of the storage flux at the next time step (which we want to calculate), $θ_{t}$ is the potential temperature at the current time step (which is generally known), $θ_{t+\Delta t}$ is the potential temperature at the next time step and $\Delta t$ is the time step itself.

<div class="question">
8.Define the storage flux function below, call it 'storageflux' and add the storageflux function to your transient model. Use a function called 'oldstate(x)' that retreives the state values of the current time step which takes distance as an argument.
</div>
...
<div class="answer">
```{r}
# The storage flux is dependent on position and state. oldstate(x) is not defined yet, we can already define the storage flux function but we are not able to call it without defining the oldstate function. The oldstate function returns the state of the system at specific location, x, at the current timestep. This information is needed in order to calculate the gradient of the state in time. dt still needs to be determined too.
S = 1
...
•	a function that is called oldstate and that returns the value of the state at the current time step at location x
