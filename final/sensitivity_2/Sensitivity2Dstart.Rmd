...
output:
   html_document:
    ...
    toc: yes
---
**Note that knitting this document can be time-consuming!**
...
## 1.1 Investigating the water balance

<div class="exercise">
1.Calculate the water balance of the whole model region. Discuss the relative contribution of the different fluxes in the system.
</div>
...
```{r}
hist(EF$river,75,main="Histogram of river fluxes in the domain",xlab="Flux [m/d]")
```
...
<div class="exercise">
3.Replace the XXXX in the following chunk by three values to draw contour lines, such that the plot gives a good view of the most frequent river fluxes. Discuss what you see.
</div>
...
In what follows we want to investigate the groundwater level in the housing area (which will be our objective, `MRESULT`). For that we need a node that falls within this area. Use the following plot to select a typical node in the housing area (zoom if needed).
```{r}
...
print(surface[180,]) #this shows the surface level at the housing area, select column that shows elevation
print(XXXX) #use previous line and combine with selected cell to obtain depth of grw below surface
```
</div>
...
<div class="exercise">
7.In section 2.2 to 2.7, calculate the local sensitivities to the other six parameters. Do this both for the MRESULT and for all the states. For each of the parameters, add a short discussion of the results.
</div>
...
# 3. LSA: relative contribution of the parameters
...
```
<div class="exercise">
8.Add a discussion of the relative contribution of the parameters to the variation in MRESULT. Is it as you expected?
</div>
...
<div class="answer">
The variation in the groundwater level of the housing area as induced by uncertainty in parameters and boundary conditions, is about 10 cm. Clearly the recharge is the dominant source of variation, followed by the transmissivity and river resistance.
</div>
...
<div class="exercise">
9.In the following sections 3.3 to 3.9, the relative contributions of the parameters to the variation in the groundwater levels is calculated. The first example for Qrch is already given. Finish the analysis for the other parameters.    
In section 3.10, add a discussion about how the contributions of all the parameters are distributed over the domain.
</div>
...
<div class="exercise">
10.In section 4.1 and 4.2, recalculate the local sensitivities of the groundwater levels (states) the way you did in section 2.1-2.7.    
In section 4.1 you should create a new base run that includes the new river bed resistance. In section 4.2, recalculate the local sensitivities of the groundwater levels of the model. There is no need to make all the local sensitivity plots again.
Finally, analyse the effects of the changed base value in section 4.3-4.5 (see exercises there).
</div>
...
<div class="exercise">
12.Plot the relative contribution of river.resist to the variation in the groundwater levels over the domain. Compare them with the previous results (section 3.6), and shortly discuss your observations.
</div>
...
The relative contribution of the river resistance has increased, especially in the upstream area of the river. Here, it causes almost all of the variation in groundwater levels.
</div>

## 4.5 contribution of river.resist to variation in M
<div class="exercise">
13.Calculate the relative contribution of river.resist to the variation in MRESULT and describe the changes. Plot the results in a pie chart.
</div>
...
<div class="exercise">
14.In the chunk below, choose sensible boundaries for the parameters and make the parameter sets.
</div>
...
<div class="exercise">
15.Run the model for your five parameter samples and save the Mvalue of each (this is best done in a loop).    
Make a plot or histogram of the Mvalues to get a first idea of the variation.
</div>
...
```
The M values (groundwater level in the housing area) more or less center around their base value, but it is not a nice distribution. This sample size probably does not give an accurate estimates of the standard deviation in M.
</div>
...
<div class="exercise">
16.Plot and analyse the conditional expectations of M with relation to each of the parameters (you can have a look at the first sensitivity assignment to see how to do it).
</div>
...
<div class="exercise">
17.Finish the analysis to find the relative contributions of each of the parameters to the variance in M.
</div>
...
## 5.6 Discussion
<div class="exercise">
18.Finally, desribe your conclusions of the GSA analysis in section 5. Do the results differ from the local sensitivity analysis? What is the effect of the parameter sample size? Do you think the parameter set of 50 is large enough?
</div>
...
Looking at the relative contributions of the parameters, Qrch turns out to be dominant, just like before in the local sensitivity analysis. However, the kD now dominates over the river resistance and drainage resistance, while in the local sensitivity they had about equal contributions. This might be influenced by the small parameter sample size (50 is still a very small sample).
</div>

<div class="comment">
COMMENTS:
Ik heb de GSA nu opgelost door ze het zelf te laten doen met een kleine sample en dan de grote sample te geven.
Het knitten van het hele document duurt nog steeds wel even, maar ik denk niet dat er nog veel aan te doen is. [LM]: Ok, thnx. 
</div>
