---
title: "Sensitivity Assignment 2 - 2D systems"
...
In what follows, we want to investigate groundwater level in the housing area (which will be our objective, `MRESULT`). For that, we need a node that falls within this area. Node number 353 falls withing the housing area, as you can check with the following plot (zoom if needed).
```{r}
...
<div class="exercise">
The state that `MRESULT` now returns is groundwater level. To get a feeling for the local impact of groundwater, we also want to know what the groundwater depth is - that is, the depth of the groundwater below the surface. The code below ('surface[180,]') provides the x, y, and z-coordinate of the housing area. Use this information to convert groundwater level to groundwater depth. What can you say about the groundwater depth?
</div>

<div class="student_answer">
```{r eval=FALSE}
#Convert to groundwater depth below the surface
print(surface[180,]) # this shows the surface level at the housing area: select the column that shows elevation
print(XXXX) #use previous line and MRESULT to obtain the depth of groundwater below the surface
```
</div>
...
```
The depth of the groundwater below the surface is about 24 centimeters; these are pretty wet conditions! To put the results into perspective; generally, farmers prefer a groundwater depth of about minimum 40 cm for grass lands and 60 cm for agricultural fields. 
</div>
...
<div class="exercise">
In section 2.2 to 2.7, calculate the local sensitivities to the other six parameters. Do this both for MRESULT and for all the states. For each of the parameters, add a short discussion of the results.
</div>

## 2.2 Sensitivity with respect to the extraction well - Qwell
...
<div class="answer">
The variation in the groundwater level of the housing area as induced by uncertainty in parameters and boundary conditions, is about 10 cm. Clearly the recharge is the dominant source of variation, followed by the transmissivity and river resistance. That uncertainty in the recharge is important, could have been expected by the fact that recharge is by far the largest flux, as discussed in 1.1 (although the parameter describing the largest overall flux does not always have to be the most sensitive parameter, this also depends on the area of interest as the next sections show, and the scale of variation of this parameter). 
</div>
...
<div class="exercise">
In sections 3.3 to 3.9, the relative contributions of the parameters to the variation in the groundwater levels over the complete domain is calculated. The first example for Qrch is already given. Finish the analysis for the other parameters. In section 3.10, add a discussion about how the contributions of all the parameters are distributed over the domain.
</div>
...
<div class="exercise">
In section 4.1 and 4.2, recalculate the local sensitivities of the groundwater levels (states) the way you did in section 2.1-2.7. In section 4.1 you should create a new base run that includes the new river bed resistance. In section 4.2, recalculate the local sensitivities of the groundwater levels of the model (there is no need to make all the local sensitivity plots again). Finally, analyse the effects of the changed base value in section 4.3-4.5 (see exercises there).
</div>
...
<div class="exercise">
Plot the relative contribution of river.resist to the variation in the groundwater levels over the domain. Compare them with the previous results (section 3.6), and shortly discuss your observations.
</div>
...
# 5. Global Sensitivity Analysis (GSA)
