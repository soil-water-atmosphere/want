...
<span class="question">
1.1.2 In order to refresh your mathematics, calculate the analytical solution to equation 1, so we can use it later to validate your models against. Use the following initial condition $s(t = 0) = s_{0}$
</span>
...
<span class="question">
1.2.3 Vary the `dt` parameter between 0.001 and 25. Are there any numerical problems arising beyond a certain value for `dt`? What are the problems when choosing a value for `dt` close to or on the high/low end of this range? Can you interpret your results in terms of convergence, oscillations and stability?
</span>
...
<span class="comment">**[LM]** expansion</span>
...
<span class="question"> 
3.1.2 Copy `chunk3` here and add a new, updated `sys.fun` that represents the equation derived in question 3.1.1. Replace the parameter section with the following:
...
$$\frac{dV}{dt} = A\dfrac{ds}{dt} = f(t, s(t)) \tag{15}$$
...
$$\frac{dV}{dt} = A\dfrac{ds}{dt} = f(t+\Delta t, s(t + \Delta t)) \tag{16}$$
...
<span class="question"> 
4.1.2 Copy the previous chunk with the explicit scheme here and replace the `sys.fun` with the implicit version.
</span>
...
$$ g_{explicit}(t,s(t)) = \widetilde{s} = s(t) + \dfrac{dt}{2A}((\sin(t) + 1) - \alpha s(t)) \tag{19}$$
```{}
