...
In the unsaturated zone above the water table, the situation is different than in our previous saturated zone model:   
*   The hydraulic conductivity is no longer constant but dependent on moisture content;   
...
*   $\theta(z)$ : moisture content (cm3/cm3)  
...
Note that in unsaturated zone modelling we generally work with units of cm and days. The reference level of the elevation head is up to the modeller to choose. In this practical, we will set $z=0 cm$ at the bottom of our domain, which coincides with the groundwater level. We will follow the (historical) convention to choose the z-direction upwards.
...
<div class="exercise">
11.Ponding happens when water stagnates on the surface. This can occur when the infiltration capacity of the topsoil is insufficient. Another reason can be that the maximum water storage capacity of the soil is exceeded, so that the soil is saturated to the surface.    
Fill out in the following table the maximum downward flux the soil can transport downwards without ponding. You can use the above code chunk, but it is not necessary. 
</div>
