...
### Hydraulic and Pressure head in the Saturated Zone
...
### Hydraulic and Pressure head in the Unsaturated Zone
...
**[GB plotje onder waterspiegel klopt niet, z=0 op maaiveld wordt in de opdrachten gehanteerd]**
...
The balance equation, which is actually the Richards equation is now:
$$
...
As illustrated in the derivation of the Richards equation the hydraulic head $H$ is replaced het pressure head $\psi$. So the state of these models will be $\psi$ which is very practical since it's now relative to the domain and boundaries (and boundary conditions) of the model. In other words, a simulation with a loamy sand in the Flevopolder in the Netherlands will have the same results for the same situation in the Alps in Switzerland.
In these assignments we will choose $\psi(z)$ as state variable. This is more practical than $H(z)$, $\psi(z)$ is independent of the choice of the reference level for $z$ and because $\psi(z)$ directly reflects the wetness of the soil.
...
<div class="answer">
the hydraulic conductivity. For the saturated zone a constant $k_{sat}$ is used. For the unsaturated zone the hydraulic conducitivty is a function of pressure(suction) $k(\psi)$ or moisture content $k(\theta)$ as will be shown later on.  
</div>

<div class="exercise">
2.Cleaning the workspace, loading the FVFE1D package and soil characteristics:
```{r}
rm(list = ls())
library(FVFE1D)
...
<div class="question">
Examine the abovementioned profiles for:  

1. $k_{sat}$
2. $k(\psi)$ and $\theta(\psi)$ at pF(2) which is called field capacity
3. as 2 but now for pF(4.2) which is called welting point

_field capacity_ the soil has optimal moisture conditions for vegetation  
_welting point_ the soil is too dry for the vegetation to extract water  
...
<div class="answer">
1. saturated conductivties of these soils can be inspected through the graphs above or with some code like below:  
