...
output:
  ...
  bookdown::html_document2:
    ...
    toc: yes
date: "March 2019"
subtitle: Groundwater flow towards rivers and Heat transfer in the atmospheric boundary
  layer
---
...
<div class="content">
This document is a so called r-markdown document, which generates a html document and contains:  

1.  different chapters and sections 
2.  code chunks
3.  questions and assignments
4.  answers and worked out assignments  

The different chapters and sections cover different aspects of the material in this document  
In the code chunks a specific piece of code can be run.  
Questions and assignments are colored red and the answers and worked out assigments are in blue.  
The 'answer' version will be available on Brightspace when suitable.  

This docouments is meant to be self contained, meaning that all aspects required to work with it, are included.  

Simply load this *.Rmd document into R-studio and "knit" or "preview" this document and have a look at the created html (in the previewer of R-studio or in a browser).  
with "knit" the document will run all chunks and create a html document with the output of the chunks and the text in the markdown.  
With "preview" the document will be created based on the text in the Rmd and the chunk which already have run.  
When working on the R-chunks one can run the individual chunks seperately and see the results.  However, in preview mode some cross-referencing and numbering could not work correctly. Knitting the whole document should return proper cross-referencing and numbering.

Questions on the answers can be typed dirctly into this document. The assignments, most of the time finishing code chunks, also need to be worked out in this document. With this you will have a completed set of _your_ answers and _your_ worked out assignments in this document.  
At the end of the time slot of this assignment a completely filled out document (including answers and worked out assignments)  will be available on Brightspace.  
...
<div class="answer">
For the Cauchy_BC one can simply implement the abovementioned equation: \@ref(eq:cauchy-eq).
...
The water balance of this model:  
...
<button class="collapsible">Background info</button>
<div class="content">
**CHiel hier graag uitleg over dit stromingsprobleem 'for dummies'**  
_internal flux_  

aanvullend; in assignment 1 "this flux is a flux of energy" met de eenheid is Km/s.  

Is het niet aardig om deze 'energy' in de eenheid terug te 'zien'. M.a.w. hoe kom je van Km/s naar J? Je zal vast constanten moeten gebruiken van vocht en lucht om te berekenen hoeveel energie het kost om zeg 1 $m^3$ lucht 1 graad te verwarmen maar de details mis ik hier een beetje.  
_external flux_  
