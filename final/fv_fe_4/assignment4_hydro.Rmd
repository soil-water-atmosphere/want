...
In this section we will focus on the effect of implementing a river in the already developed 2D groundwater model `GWModel2D` model.  
...
3. as illustrated by the state (head) distribution, water flows from the centre of the model towards the western and eastern boundaries. 
</div>
...
<div class="question">
How would you call this kind of boundary condition?  
</div>
...
The river level is prescribed: 1.25 m (Above Mean Sea Level)  
The entrance of the river bed: 5 days  
...
<div class="question">
Set up a new function for the river in the following chunk
</div>
...
In one of the exercises in the previous section line flux a variable called `river` was already defined.  
We will make use of this variable again. It is setup a matrix as shown in the chunk below:  
...
<div class="question">
Define the river in a proper way to be added to the model in the following chunk  
```{r}
#add river to model
#add.lineflux()
#summary(TwoDmodelRiver)
...
<div class="question">
set up the following chunk for running the model, plot the stated and to show the balance of the model
```{r}
#run the model
...
```
<div class="question">
Calculate "by hand", the discharge rate to the river in this node. Check your results with the (water) balance for this node.
Total total length $l$ of the river allocated to node 252 is 120.0829 m
</div>

<div class="answer">
The river level, the river-bed resistance and the width of the river are all given.
...
<div class="question">
Calculate "by hand" the internal flux over the side of node 252 and 230.  

Besides the data already present, additional data is available in the following table:  

* the `facelength` between node 252 and `ni2`.  
* the `nodedistances` between node 252 and `ni2`.  
...
<div class="answer">
Based on the above data one can derive the internal flux over the face between node 252 and 230 by:  

* states of both nodes  
* transmissivity: 45 $m^2/d$  
* Darcy's equation
* `facelength`
* `nodedistance`  
...
<div class="exercise">
20. Define the model domain in a new R-code chunk, as illustrated in figure 3, and plot the new domain. Firstly, clear the defined objects from the work space to prevent unwanted interference with previous model simulations. What is the y-to-x-ratio in this example?
</div>
...
<div class=answer>
20. The y-to-x-ratio in this example is 5.
```{r}
#rm(list=ls())
...
The hydraulich conductivity depends on the pressure head and not the hydraulic head which is currently the state.
...
We will use these ready made functions which are set in the (binary) data set `Staring.Rdata`. 


some parameter for a few soil types:  

Soil type |Soil code| $\theta_{res}$| $\theta_{sat}$| n   | $\alpha$| $\lambda$| $K_{sat}$  
Loamy sand| B2      | 0.02          | 0.42          | 1.49| 0.0276  | -1.06    | 12.5
...
<div class="answer">
22. The hydraulic conductivity is a constant $k_{sat}$ for saturated conditions but a function of pressure $\psi$ and highly non-linear for unsaturated conditions, see FVFE assignment 3 for details on this.  
...
23. Using the ratio gives an evenly distribution of nodes in the domain although length and height scale are different.
```{r}
...
<div class="answer">
24. The largest flux is observed at the lower west boundary, where all the water is forced through a relatively small surface. The highest flux density is observed here. The length of these vectors represent the magnitude of the flux. The arrows point in the direction of the flow.
```{r}
# Solve
solve.steps(DikeFlow, verboselevel = 1)
...
25. Since $H=\psi + z \rightarrow \psi = H-z$  
...
<div class="exercise">
26. Last exercise; play around with the dike model. Investigate how you, with the help of the model that you have built, can investigate the flow through a dike for the following examples:
...
26.c  
This is simply accomplished by increasing the amount of recharge on top of the dike.  
...
When the recharge rate is increased to 10 mm/d one can clearly see the water table rise to more than 2/3 of the dike (about 1.5 m above datum).  
</div>
