...
<div class="answer">
This should remind you of the FV principle. The same state is assigned to all positions within the volume. A blocky behavior is found. 
</div>
...
To adjust for this segment with ID= 2 need to be cut in a lower and upper part and the model need to be recreated on with this.
...
26.b  
To include an extra layer with high conductivity hydraulic conductivity need to be transformed into a function based on interpolation.
...
When the recharge rate is increased to 10 mm/d one can clearly see the water table ($\psi=0$) risen to more than half way the dike
</div>
</div>